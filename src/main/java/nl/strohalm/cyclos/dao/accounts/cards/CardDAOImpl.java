...
public class CardDAOImpl extends BaseDAOImpl<Card> implements CardDAO {
    ...
    public void cancelAllMemberCards(final Member member) {
        ...
        executeUpdate("update " + getEntityType().getName() + " set status = :status where owner = :owner ", namedParameters);
    ...
    public Card seachLastCard(final long memberId) throws DaoException {
        final Map<String, Object> namedParameters = new HashMap<String, Object>();
        final StringBuilder hql = new StringBuilder("select c from " + Card.class.getName() + " c");
        hql.append(" where c.owner.id = :memberId ");
        hql.append(" and c.creationDate = ( select max(c1.creationDate) from " + Card.class.getName() + " c1 ");
        hql.append(" where c1.owner.id = :mId )");
        namedParameters.put("memberId", memberId);
        namedParameters.put("mId", memberId);
