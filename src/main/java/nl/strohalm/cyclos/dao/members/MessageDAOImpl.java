...
public class MessageDAOImpl extends BaseDAOImpl<Message> implements MessageDAO {
    ...
    public Member getNextMemberToSend(final Message message) {
        return (Member) getSession().createFilter(message.getPendingToSend(), "where 1=1").setMaxResults(1).uniqueResult();
    }

    public List<Long> listUnfinishedIds() {
        return list("select m.id from Message m where exists elements(m.pendingToSend)", null);
    }
