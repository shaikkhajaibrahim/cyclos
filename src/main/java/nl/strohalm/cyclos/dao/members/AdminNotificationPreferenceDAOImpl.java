...
public class AdminNotificationPreferenceDAOImpl extends BaseDAOImpl<AdminNotificationPreference> implements AdminNotificationPreferenceDAO {
    ...
    public List<Administrator> searchAdmins(final AdminNotificationPreferenceQuery query) {
        ...
        if (query.getSystemAlert() != null) {
            hql.append(" and exists (select sa.id from SystemAlertNotificationPreference sa where sa.notificationPreference.id = p.id and sa.type = :systemAlertType)");
            namedParameters.put("systemAlertType", query.getSystemAlert());
        }
        if (query.getMemberAlert() != null) {
            hql.append(" and exists (select ma.id from MemberAlertNotificationPreference ma where ma.notificationPreference.id = p.id and ma.type = :memberAlertType)");
