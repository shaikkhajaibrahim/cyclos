...
public abstract class BaseDAOImpl<E extends Entity> extends HibernateDaoSupport implements BaseDAO<E>, InsertableDAO<E>, UpdatableDAO<E>, DeletableDAO<E> {
    ...
    public <T extends E> T update(final T entity, final boolean flush) {
        ...
        try {
            hibernateQueryHandler.resolveReferences(entity);
            final T ret = (T) getHibernateTemplate().execute(new HibernateCallback() {
    ...
    protected void evictSecondLevelCache() {
        // Evict the collection regions
        if (shouldEvictSecondLevelCacheForCollections()) {
     ...
     */
    protected int executeUpdate(final String hql, final Object namedParameters) {
        try {
            final int records = getHibernateTemplate().execute(new HibernateCallback<Integer>() {
            ...
            if (records > 0) {
                // For bulk updates, notify the write, as there are no event listeners for this
    ...
    protected <T> Iterator<T> iterate(final String hql, final Object namedParameters) {
        try {
            return hibernateQueryHandler.simpleIterator(hql, namedParameters, null, null);
        } catch (final ApplicationException e) {
            throw e;
        } catch (final Exception e) {
            throw new DaoException(e);
    ...
    /**
     * Can be overridden in order to determine whether saving / removing an entity will cause all 2nd level cache collection regions
     */
    ...
    protected boolean shouldUseQueryCache() {
        // By default, use the same as defined in the shouldEvictSecondLevelCacheForCollections()
        return shouldEvictSecondLevelCacheForCollections();
    }
