...
public class LocalSettings extends DataObject {
    ...
    public static enum Language implements StringValuedEnum {
        ...
        GERMAN("de_DE", "de", new GermanAnalyzer(IndexHandler.LUCENE_VERSION)),

        DUTCH("nl_NL", "nl"),

        PORTUGUESE_BRAZIL("pt_BR", "pt-br", new BrazilianAnalyzer(IndexHandler.LUCENE_VERSION)),

        CHINESE_SIMPLIFIED("zh_CN", "zh-cn"),

        FRENCH("fr_FR", "fr", new FrenchAnalyzer(IndexHandler.LUCENE_VERSION)),
        ...
        private Language(final String value, final String alternate) {
            this(value, alternate, new StandardAnalyzer(IndexHandler.LUCENE_VERSION));
        }
    ...
    /**
     * Returns whether the notifications by SMS are enabled
     */
    public boolean isSmsNotificationEnabled() {
        return StringUtils.isNotEmpty(sendSmsWebServiceUrl);
    }
    ...
    public void setCharset(final String encoding) {
        charset = encoding;
    }
