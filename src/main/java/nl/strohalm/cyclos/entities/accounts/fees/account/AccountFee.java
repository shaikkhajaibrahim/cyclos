...
public class AccountFee extends Entity {
    ...
    /**
     * Create a log for the next execution of this account fee
     */
    public AccountFeeLog calculateNextExecution() {
        if (!enabled || runMode == RunMode.MANUAL) {
            return null;
        }

        final TimePeriod recurrence = getRecurrence();
        ...
        // Resolve the date / time this log would run
        final Calendar executionDate = DateHelper.truncateNextDay(period.getEnd());
        if (day != null) {
            executionDate.set(Calendar.DATE, day);
        }
        if (hour != null) {
        ...
        log.setStatus(AccountFeeLog.Status.SCHEDULED);
        log.setAccountFee(this);
        log.setDate(executionDate);
        log.setPeriod(period);
        log.setAmount(getAmount());
        log.setFreeBase(getFreeBase());
        log.setTolerance(getTolerance());
        return log;
    }
    ...
    public AccountFeeLog getLastExecution() {
        for (final AccountFeeLog accountFeeLog : getLogs()) {
            // Scheduled status are not counted
    ...
    public AccountFeeLog getNextExecution() {
        // Find the scheduled log
        for (final AccountFeeLog accountFeeLog : getLogs()) {
            if (accountFeeLog.getStatus() == AccountFeeLog.Status.SCHEDULED) {
                return accountFeeLog;
            }
    ...
    public WeekDay getWeekDay() {
        if (recurrence != null && recurrence.getField() == TimePeriod.Field.WEEKS) {
            return WeekDay.valueOf(recurrence.getNumber());
        }
        return null;
    }
