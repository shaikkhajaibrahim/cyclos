...
public class Guarantee extends Entity implements EntityWithCustomFields<PaymentCustomField, PaymentCustomFieldValue> {
    ...
    protected void appendVariableValues(final Map<String, Object> variables, final LocalSettings localSettings) {
        ...
        variables.put("buyerName", getBuyer().getName());
        variables.put("buyerUsername", getBuyer().getUsername());
        if (getSeller() != null) {
            variables.put("sellerName", getSeller().getName());
            variables.put("sellerUsername", getSeller().getUsername());
        }
        if (getIssuer() != null) {
            variables.put("issuerName", getIssuer().getName());
            variables.put("issuerUsername", getIssuer().getUsername());
        }
    }
