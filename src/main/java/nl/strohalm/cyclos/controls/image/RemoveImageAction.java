    ...
    protected void renderContent(final ActionContext context) throws Exception {
        ...
        try {
            ...
            switch (image.getNature()) {
                case AD:
                    if (context.getAccountOwner().equals(((AdImage) image).getAd().getOwner())) {
                        imageService.removeMyAdImages(id);
                    } else {
                        imageService.removeAdImages(id);
                    }
                    break;
                case MEMBER:
                    if (context.getAccountOwner().equals(((MemberImage) image).getMember())) {
                        imageService.removeMyImages(id);
                    } else {
                        imageService.removeMemberImages(id);
                    }
                    break;
                default:
                    throw new ValidationException();
            ...
            // Remove the image from the disk cache
            ImageByIdServlet.removeFromCache(context.getServletContext(), id);
        }
    }
