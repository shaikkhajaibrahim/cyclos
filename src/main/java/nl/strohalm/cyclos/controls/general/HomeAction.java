...
public class HomeAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        if (getAccessService().hasPasswordExpired(context.getUser())) {
            return context.findForward("changeExpiredPassword");
        }
        ...
        if (context.isTransactionPasswordEnabled()) {
            ...
            if (loggedGroup instanceof OperatorGroup) {
                loggedGroup = getFetchService().fetch(loggedGroup, RelationshipHelper.nested(OperatorGroup.Relationships.MEMBER, Element.Relationships.GROUP));
            }
        ...
        if (context.isAdmin()) {
            // Admin specific data
            if (getPermissionService().checkPermission("systemStatus", "view")) {
