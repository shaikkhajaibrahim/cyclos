...
public class MobileActionContext extends AbstractActionContext {
    ...
    public MobileActionContext(final ActionMapping actionMapping, final ActionForm actionForm, final HttpServletRequest request, final HttpServletResponse response, final MemberUser user) {
    ...
    public UnitsConverter getUnitsConverter() {
        final LocalSettings settings = SettingsHelper.getLocalSettings(getServletContext());
        return settings.getUnitsConverter(getCurrentAccountType().getCurrency().getPattern());
    }
