...
public class MobileDoPaymentAction extends MobileBaseAction {
    ...
    private BigDecimal validateAmount(final MobileActionContext context) {
        ...
        final UnitsConverter converter = context.getUnitsConverter();
    ...
    private String validateDescription(final MobileActionContext context) {
        final MobileDoPaymentForm form = context.getForm();
        final String description = StringUtils.trimToNull(form.getDescription());
        if (description == null) {
            throw new MobileException("errors.required", context.message("mobile.payment.description"));
        }
        return description;
    }
