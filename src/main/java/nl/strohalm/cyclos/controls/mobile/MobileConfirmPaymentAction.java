...
public class MobileConfirmPaymentAction extends MobileBaseAction {
    ...
    protected ActionForward executeAction(final MobileActionContext context) throws Exception {
        ...
        if (RequestHelper.isGet(request)) {
            ...
            final UnitsConverter unitsConverter = context.getUnitsConverter();
            final TransferType transferType = payment.getTransferType();
            ...
            if (requestTransactionPassword) {
                ...
                try {
                    accessService.checkTransactionPassword(null, context.getUser().getUsername(), transactionPassword, request.getRemoteAddr());
            ...
            // Perform the payment
            try {
                paymentService.doPaymentFromMemberToMember(payment);
    ...
    private DoPaymentDTO validatePayment(final HttpSession session) {
        ...
        if (payment == null || payment.getAmount().compareTo(new BigDecimal(0)) != 1 || (!(payment.getTo() instanceof Member)) || payment.getDescription() == null || payment.getTransferType() == null) {
