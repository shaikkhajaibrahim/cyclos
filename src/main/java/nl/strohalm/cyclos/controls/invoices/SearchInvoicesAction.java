...
public class SearchInvoicesAction extends BaseQueryAction implements LocalSettingsChangeListener {
    ...
    public class TransformInvoiceInEntry implements Transformer<Invoice, Entry> {
        ...
        private AccountOwner  loggedOwner;
        private LocalSettings localSettings;

        public TransformInvoiceInEntry(final ActionContext context) {
            loggedOwner = context.getAccountOwner();
            localSettings = SettingsHelper.getLocalSettings(context.getServletContext());
        }
        ...
        public Entry transform(final Invoice invoice) {
            ...
            final AccountOwner related = loggedOwner.equals(from) ? to : from;
    ...
    protected void checkPermission(final ActionContext context) {
        ...
        final boolean hasMemberId = form.getMemberId() > 0;

        if (hasMemberId) {
            final Member member = (Member) elementService.load(form.getMemberId(), Element.Relationships.GROUP);
            requestor.manages(member);

            requestor.adminPermissions("adminMemberInvoices", "view");
        } else {
            requestor.adminPermissions("adminMemberInvoices", "accept", "deny", "cancel");
        }
        ...
        if (!getPermissionService().checkPermissions(requestor)) {
    ...
    protected void executeQuery(final ActionContext context, final QueryParameters queryParameters) {
        ...
        final TransformInvoiceInEntry transformer = new TransformInvoiceInEntry(context);
    ...
    protected QueryParameters prepareForm(final ActionContext context) {
        ...
        if (owner instanceof SystemAccountOwner) {
            ...
            member = getFetchService().fetch((Member) owner, Element.Relationships.USER);
        ...
        if (query.getRelatedMember() instanceof EntityReference) {
            query.setRelatedOwner(getFetchService().fetch(query.getRelatedMember(), Element.Relationships.USER));
        }
        if (query.getTransferType() instanceof EntityReference) {
            query.setTransferType(getFetchService().fetch(query.getTransferType()));
        }
