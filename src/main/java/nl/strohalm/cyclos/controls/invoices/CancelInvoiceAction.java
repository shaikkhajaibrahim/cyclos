...
public class CancelInvoiceAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        final Invoice invoice = invoiceService.load(id, Invoice.Relationships.FROM_MEMBER);
        final boolean asMember = !context.getAccountOwner().equals(invoice.getFrom());
        if (asMember) {
            invoiceService.cancelAsMember(invoice);
        } else {
            if (invoice.isFromSystem()) {
                invoiceService.cancelSystem(invoice);
            } else {
                invoiceService.cancelMember(invoice);
            }
        }
