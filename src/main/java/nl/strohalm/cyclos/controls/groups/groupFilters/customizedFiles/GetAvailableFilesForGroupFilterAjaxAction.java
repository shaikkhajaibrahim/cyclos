...
import nl.strohalm.cyclos.utils.conversion.JavaScriptStringEscapeConverter;
...
public class GetAvailableFilesForGroupFilterAjaxAction extends BaseAjaxAction {
    ...
    public SimpleCollectionBinder<String> getCollectionBinder() {
        if (collectionBinder == null) {
            collectionBinder = SimpleCollectionBinder.instance(String.class, JavaScriptStringEscapeConverter.instance());
        }
    ...
    protected void renderContent(final ActionContext context) throws Exception {
        ...
        final List<String> notAlreadyCustomized = CustomizationHelper.onlyNotAlreadyCustomized(getServlet().getServletContext(), type, customizedFiles);
