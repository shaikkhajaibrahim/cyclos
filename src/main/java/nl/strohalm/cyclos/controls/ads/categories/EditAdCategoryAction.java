...
public class EditAdCategoryAction extends BaseFormAction {
    ...
    public DataBinder<AdCategory> getDataBinder() {
        if (dataBinder == null) {
            ...
            binder.registerBinder("countOffer", PropertyBinder.instance(Integer.class, "countOffer"));
            binder.registerBinder("countSearch", PropertyBinder.instance(Integer.class, "countSearch"));
            dataBinder = binder;
        }
        return dataBinder;
    }
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        if (insert) {
            ...
            for (String name : names) {
                ...
                final AdCategory cat = (AdCategory) category.clone();
                cat.setName(name);
                adCategoryService.save(cat);
                if (count == 1) {
                    id = cat.getId();
                }
            }
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        request.setAttribute("editable", getPermissionService().checkPermission("systemAdCategories", "manage"));
