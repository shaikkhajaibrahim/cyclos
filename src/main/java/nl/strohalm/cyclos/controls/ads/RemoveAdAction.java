...
public class RemoveAdAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        final Ad ad = adService.load(form.getId(), Ad.Relationships.OWNER);
        final AccountOwner loggedOwner = context.getAccountOwner();
        if (loggedOwner.equals(ad.getOwner())) {
            adService.removeMyAd(ad.getId());
        } else {
            adService.removeMemberAd(ad.getId());
        }
