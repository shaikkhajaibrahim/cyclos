...
public class EditAdAction extends BaseAdAction {
    ...
    protected int getNumberOfAds(final Member member) {
        final Map<Ad.Status, Integer> adMap = getAdService().getNumberOfAds(member);
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        if (ad.getOwner() == null) {
            ...
            if (context.isOperator()) {
                ...
                myAd = ad.getOwner().equals(operator.getMember());
            } else { // context.isAdmin() || context.isMember()
                myAd = ad.getOwner().equals(context.getElement());
            }
        }

        if (myAd) {
            ad = getAdService().saveMyAd(ad);
        } else {
            ad = getAdService().saveMemberAd(ad);
        }
        ...
        if (upload != null && upload.getFileSize() > 0) {
            try {
                ...
                if (myAd) {
                    getImageService().saveMyAdImage(ad, form.getPictureCaption(), ImageType.getByContentType(upload.getContentType()), newFileName.toString(), upload.getInputStream());
                } else {
                    getImageService().saveAdImage(ad, form.getPictureCaption(), ImageType.getByContentType(upload.getContentType()), newFileName.toString(), upload.getInputStream());
                }
            } finally {
                upload.destroy();
            }
