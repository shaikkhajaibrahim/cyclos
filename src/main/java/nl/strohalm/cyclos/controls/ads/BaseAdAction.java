...
public abstract class BaseAdAction extends BaseFormAction implements LocalSettingsChangeListener {
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (context.isAdmin()) {
            ...
            if (loggedMember.equals(ad.getOwner())) {
                editable = true;
            } else {
                editable = context.isBrokerOf(ad.getOwner()) && getPermissionService().checkPermission("brokerAds", "manage");
            }
        } else { // context.isOperator()
            final Operator operator = context.getElement();
            if (operator.getMember().equals(ad.getOwner())) {
                editable = true;
            }
        }
