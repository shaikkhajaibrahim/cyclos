...
public abstract class ProfileAction<E extends Element> extends BaseFormAction implements LocalSettingsChangeListener {
    ...
    protected void updateLoggedUser(final Element element, final ActionContext context) {
        if (context.getElement().equals(element)) {
            final User user = getFetchService().reload(element.getUser(), RelationshipHelper.nested(User.Relationships.ELEMENT, Element.Relationships.GROUP));
            if (user instanceof OperatorUser) {
                // Fetch the member also
                final Operator operator = (Operator) user.getElement();
                final Member member = getFetchService().reload(operator.getMember(), Element.Relationships.GROUP);
                operator.setMember(member);
            }
            context.getSession().setAttribute("loggedUser", user);
        }
    }
}
