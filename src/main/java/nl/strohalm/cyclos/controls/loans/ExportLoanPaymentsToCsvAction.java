...
public class ExportLoanPaymentsToCsvAction extends BaseCsvAction {
    ...
    public DataBinder<LoanPaymentQuery> getDataBinder() {
        try {
            lock.readLock().lock();
            if (dataBinder == null) {
                dataBinder = SearchLoanPaymentsAction.loanPaymentQueryDataBinder(getLocalSettings());
            }
            return dataBinder;
        } finally {
    ...
    protected List<?> executeQuery(final ActionContext context) {
        ...
        query.fetch(RelationshipHelper.nested(LoanPayment.Relationships.LOAN, Loan.Relationships.TRANSFER, Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER, Element.Relationships.USER));
