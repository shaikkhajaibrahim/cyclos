...
public class EditCertificationAction extends BaseFormAction {
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (!isInsert) {
            ...
            final ByElementExtractor extractor = new ByElementExtractor() {
                public Element getByElement(final Entity entity) {
                    return getFetchService().fetch(((CertificationLog) entity).getBy());
                }
            };

            final boolean showGuarantees = !context.isAdmin() || getPermissionService().checkPermission("adminMemberGuarantees", "viewGuarantees");
        ...
        if (isIssuer) { // only if the logged user is a issuer we must search for guarantee types
            ...
            group = getFetchService().fetch(group, Group.Relationships.GUARANTEE_TYPES);
