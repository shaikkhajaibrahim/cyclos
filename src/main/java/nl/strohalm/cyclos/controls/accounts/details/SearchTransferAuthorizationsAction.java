...
public class SearchTransferAuthorizationsAction extends BaseTransferAuthorizationSearchAction {
    ...
    protected QueryParameters prepareForm(final ActionContext context) {
        ...
        query.fetch(TransferAuthorization.Relationships.BY, RelationshipHelper.nested(TransferAuthorization.Relationships.TRANSFER, Transfer.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(TransferAuthorization.Relationships.TRANSFER, Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER));
        ...
        if (memberId > 0L) {
            ...
            if (query.getBy() != null) {
                by = getFetchService().fetch(query.getBy(), Element.Relationships.USER);
                query.setBy(by);
            }
        }
