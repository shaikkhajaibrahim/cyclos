...
public class SearchPosAction extends BaseQueryAction {
    ...
    /**
     * Check wether the broker or admin manages the member or group for the POS.
     * @param context
     * @param pos
     * @param fetchService
     */
    public static boolean canManagePos(final ActionContext context, final Pos pos, final FetchService fetchService) {
        if (pos.getStatus() == Pos.Status.DISCARDED) {
            return false;
        } else if ((pos.getMemberPos() != null)) {
            if (context.isBrokerOf(pos.getMemberPos().getMember())) {
                return true;
            } else if (context.isAdmin()) {
                final AdminGroup group = (AdminGroup) fetchService.fetch(context.getGroup(), AdminGroup.Relationships.MANAGES_GROUPS);
                return group.getManagesGroups().contains(pos.getMemberPos().getMember().getGroup());
            } else {
                return false;
            }
        } else {
