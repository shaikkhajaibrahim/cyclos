...
public abstract class BaseTransferAuthorizationSearchAction extends BaseQueryAction {
    ...
    protected List<TransferType> resolveTransferTypes(Element element) {
        element = getFetchService().fetch(element, Element.Relationships.GROUP);
        ...
        if (element instanceof Administrator) {
            ...
            if (element instanceof Operator) {
                ...
                member = getFetchService().fetch(operator.getMember(), RelationshipHelper.nested(Element.Relationships.GROUP, MemberGroup.Relationships.ACCOUNT_SETTINGS));
            } else {
                member = getFetchService().fetch((Member) element, RelationshipHelper.nested(Element.Relationships.GROUP, MemberGroup.Relationships.ACCOUNT_SETTINGS));
            }
