...
public class EditPosAction extends BaseFormAction {
    ...
    private class PosEditionPermissions {
        ...
        public boolean isAllowed(final String operation) {
            if (operation == null) {
                return true;
            } else if ("block".equals(operation) && !isCanBlock()) {
                return false;
            } else if ("unblock".equals(operation) && !isCanUnblock()) {
                return false;
            } else if ("assign".equals(operation) && !isCanAssign()) {
                return false;
            } else if ("changePin".equals(operation) && !isCanChangePin()) {
                return false;
            } else if ("discard".equals(operation) && !isCanDiscard()) {
                return false;
            } else if ("unassign".equals(operation) && !isCanUnassign()) {
                return false;
            } else if ("unblockPin".equals(operation) && !isCanUnblockPin()) {
                return false;
            } else if ("newPos".equals(operation) && !isEditable()) {
                return false;
            } else if ("updatePos".equals(operation) && !isEditable()) {
                return false;
            } else {
                return true;
            }
    ...
    protected void checkPermission(final ActionContext context) {
        ...
        final PosEditionPermissions permissions = checkPermissionsInformation(context);
        final String operation = form.getOperation();
        final long posId = form.getId();
        final boolean isInsert = posId <= 0;

        final PermissionRequestorImpl permissionRequestor = new PermissionRequestorImpl();
        permissionRequestor.adminPermissions("adminMemberPos", isInsert ? "manage" : "view");
        permissionRequestor.brokerPermissions("brokerPos", isInsert ? "manage" : "view");

        if (!isInsert) {
            final Pos pos = posService.load(posId, Relationships.MEMBER_POS);
            if (pos.getMemberPos() != null) {
                permissionRequestor.member(); // allow access to the member
                permissionRequestor.manages(pos.getMemberPos().getMember());
            }
        } else if (operation != null && !operation.equals("newPos")) {
            throw new PermissionDeniedException();
        }

        if (!getPermissionService().checkPermissions(permissionRequestor) || !permissions.isAllowed(operation)) {
            throw new PermissionDeniedException();
        }
    }
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        try {
            ...
            if (operation.equals("block")) {
                ...
                final Member member = (Member) elementService.load(Long.parseLong(form.getAssignTo()));
    ...
    private boolean checkPermission(final String module, final String operation) {
        final PermissionService permissionService = getPermissionService();
        return permissionService.checkPermission(module, operation);
    }
    ...
    private PosEditionPermissions checkPermissionsInformation(final ActionContext context) {
        ...
        permissions.setEditable(checkPermission("adminMemberPos", "manage") || checkPermission("brokerPos", "manage"));
        if (isInsert) {
            ...
            if (context.isAdmin() || (context.isBroker() && !hasMemberPos) || (context.isBroker() && hasMemberPos && !member.equals(context.getElement()))) {
                String module;
                if (context.isAdmin()) {
                    module = "adminMemberPos";
                } else {
                    module = "brokerPos";
                }
                switch (pos.getStatus()) {
                    case UNASSIGNED:
                        permissions.setCanAssign(checkPermission(module, "assign"));
                        break;
                    case ASSIGNED:
                        permissions.setCanUnassign(checkPermission(module, "assign"));
                        permissions.setCanChangePin(checkPermission(module, "changePin"));
                        if (hasMemberPos) {
                            final Status status = pos.getMemberPos().getStatus();
                            if (status == MemberPos.Status.ACTIVE) {
                                permissions.setCanBlock(checkPermission(module, "block"));
                            }
                            if (status == MemberPos.Status.BLOCKED) {
                                permissions.setCanUnblock(checkPermission(module, "block"));
                            }
                            if (status == MemberPos.Status.PIN_BLOCKED) {
                                permissions.setCanUnblockPin(checkPermission(module, "unblockPin"));
                            }
                        }
                        break;
                    case DISCARDED:
                ...
                if (pos.getStatus() != Pos.Status.DISCARDED) {
                    permissions.setCanDiscard(checkPermission(module, "discard"));
                }
                permissions.setCanChangeParameters(checkPermission(module, "changeParameters"));
