...
public class ExportTransactionsToCsvAction extends BaseCsvAction {
    ...
    public DataBinder<TransferQuery> getDataBinder() {
        if (dataBinder == null) {
            dataBinder = AccountHistoryAction.transferQueryDataBinder(getLocalSettings());
        }
        return dataBinder;
    }
    ...
    protected List<?> executeQuery(final ActionContext context) {
        ...
        query.fetch(Transfer.Relationships.FROM, Transfer.Relationships.TO, Transfer.Relationships.TYPE);
        query.setResultType(ResultType.ITERATOR);
        final List<Transfer> transfers = paymentService.search(query);
        return AccountHistoryAction.Entry.build(elementService, account, transfers, false);
    }
    ...
    @Override
    protected CSVWriter<Transfer> initCsvWriter(final ActionContext context) {
        final AccountHistoryForm form = context.getForm();
        final TransferQuery query = getDataBinder().readFromString(form.getQuery());
        final Account account = getAccount(query);
        ...
        final List<PaymentCustomField> customFields = customFieldService.listPaymentFieldForList(account, false);
