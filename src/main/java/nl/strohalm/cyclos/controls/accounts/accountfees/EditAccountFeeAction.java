...
public class EditAccountFeeAction extends BaseFormAction implements LocalSettingsChangeListener {
    ...
    /**
     * Build a transfer type query for possible generated transfer types of the given account type and charge mode
     */
    public static TransferTypeQuery buildGeneratedTypeQuery(final MemberAccountType accountType, final PaymentDirection paymentDirection) {
    ...
    public DataBinder<AccountFee> getDataBinder() {
        try {
            ...
            if (dataBinder == null) {
                ...
                binder.registerBinder("tolerance", DataBinderHelper.timePeriodBinder("tolerance"));
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (isInsert) {
            ...
            final List<TransferType> transferTypes = transferTypeService.search(buildGeneratedTypeQuery(accountType, accountFee.getPaymentDirection()));
        ...
        request.setAttribute("toleranceFields", Arrays.asList(TimePeriod.Field.HOURS, TimePeriod.Field.DAYS));
        request.setAttribute("shouldConfirmWhenDisabling", accountFee.isEnabled() && accountFee.getChargeMode().isVolume());
