...
/**
 * Action used to cancel a running account fee
 * @author luis
 */
public class CancelRunningAccountFeeAction extends BaseAccountFeeExecutionAction {
    ...
    protected void executeAction(final ActionContext context, final AccountFee fee, final AccountFeeLog log) {
        ...
        try {
            getAccountFeeService().cancel(log);
            context.sendMessage("accountFee.action.canceled");
        } catch (final UnexpectedEntityException e) {
            context.sendMessage("accountFee.action.alreadyFinished");
        }
    }
}
