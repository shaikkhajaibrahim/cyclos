...
public class SearchGuaranteesAction extends BaseQueryAction {
    ...
    protected QueryParameters prepareForm(final ActionContext context) {
        ...
        final boolean hasViewPermission = getPermissionService().checkPermission("adminMemberGuarantees", "viewGuarantees");
        ...
        final Group group = getFetchService().fetch(context.getGroup(), Group.Relationships.GUARANTEE_TYPES);
        if (context.isAdmin()) {
        ...
        if (query.getGuaranteeType() != null) {
            final GuaranteeType guaranteeType = getFetchService().fetch(query.getGuaranteeType());
            customFields = customFieldService.listPaymentFields(guaranteeType.getLoanTransferType());
        } else {
            customFields = new LinkedHashSet<PaymentCustomField>();
            for (final GuaranteeType guaranteeType : guaranteeTypes) {
                customFields.addAll(customFieldService.listPaymentFields(guaranteeType.getLoanTransferType()));
            }
        }
