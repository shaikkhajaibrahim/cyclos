 ...
 * @author luis
 */
public abstract class BaseQueryAction extends BaseAction implements LocalSettingsChangeListener {
    ...
    public int getMaxIteratorResult() {
        return maxIteratorResult;
    }
    ...
    public void setServlet(final ActionServlet servlet) {
        ...
        if (servlet != null) {
            final LocalSettings settings = SettingsHelper.getLocalSettings(servlet.getServletContext());
            defaultPageSize = settings.getMaxPageResults();
            maxIteratorResult = settings.getMaxIteratorResults();
        }
    }
    ...
    protected final ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        if (isValidation(context)) {
            try {
                ...
                ResponseHelper.writeValidationSuccess(request, context.getResponse());
            } catch (final ValidationException e) {
                ResponseHelper.writeValidationErrors(request, context.getResponse(), e);
            }
            return null;
        } else {
