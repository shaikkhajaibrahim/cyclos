...
public class RemoveDocumentAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        if (document instanceof DynamicDocument) {
            documentService.removeDynamic(id);
            final DynamicDocument dynamicDocument = (DynamicDocument) document;
            if (dynamicDocument.isHasFormPage()) {
                final File formFile = CustomizationHelper.formFile(context.getServletContext(), dynamicDocument);
                formFile.delete();
            }
            final File docFile = CustomizationHelper.documentFile(context.getServletContext(), dynamicDocument);
            docFile.delete();
            forwardName = "listDocuments";
        } else if (document instanceof MemberDocument) {
            documentService.removeMember(id);
            ...
            documentService.removeStatic(id);
            forwardName = "listDocuments";
        }
