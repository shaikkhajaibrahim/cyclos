...
public class EditCustomizedFileAction extends BaseFormAction {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        final File originalFile = CustomizationHelper.originalFileOf(servletContext, customizedFile.getType(), customizedFile.getName());
        final String originalContents = FileUtils.readFileToString(originalFile);

        final File physicalFile = CustomizationHelper.customizedFileOf(servletContext, customizedFile.getType(), customizedFile.getName());
        ...
        customizedFile = customizedFileService.saveGlobal(customizedFile);

        // Physically update the file
        CustomizationHelper.updateFile(servletContext, physicalFile, customizedFile);
    ...
    private List<String> filesNotYetCustomized(final CustomizedFile.Type type) {
        ...
        return CustomizationHelper.onlyNotAlreadyCustomized(getServlet().getServletContext(), type, customizedFiles);
    }
}
