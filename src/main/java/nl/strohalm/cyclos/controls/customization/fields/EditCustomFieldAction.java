...
public class EditCustomFieldAction extends BaseFormAction {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        switch (nature) {
            case OPERATOR:
                final OperatorCustomField operatorCustomField = (OperatorCustomField) field;
                if (isInsert) {
                    final Member member = (Member) context.getElement();
                    operatorCustomField.setMember(member);
                }
                field = customFieldService.save(operatorCustomField);
                break;
            case MEMBER_RECORD:
                final MemberRecordCustomField memberRecordCustomField = (MemberRecordCustomField) field;
                field = customFieldService.save(memberRecordCustomField);
                break;
            case PAYMENT:
                final PaymentCustomField paymentCustomField = (PaymentCustomField) field;
                field = customFieldService.save(paymentCustomField);
                break;
            default:
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (id <= 0L) {
            ...
            switch (nature) {
                case OPERATOR:
                    ...
                    break;
                case MEMBER_RECORD:
                    final Long memberRecordTypeId = form.getMemberRecordTypeId();
                    final MemberRecordType memberRecordType = memberRecordTypeService.load(memberRecordTypeId);
                    ((MemberRecordCustomField) field).setMemberRecordType(memberRecordType);
                    break;
                case PAYMENT:
                    final TransferType transferType = transferTypeService.load(transferTypeId);
                    ((PaymentCustomField) field).setTransferType(transferType);
                    break;
            }
            // Retrieve the possible parent fields
            final List<? extends CustomField> possibleParentFields = customFieldService.listPossibleParentFields(field);
            request.setAttribute("possibleParentFields", possibleParentFields);
        } else {
            field = customFieldService.load(id, RelationshipHelper.nested(CustomField.Relationships.PARENT, CustomField.Relationships.POSSIBLE_VALUES), MemberRecordCustomField.Relationships.MEMBER_RECORD_TYPE);
            ...
            if (parent == null) {
                ...
                if (parentValueId > 0L) {
                    ...
                    parentValue = customFieldService.loadPossibleValue(parentValueId);
                } else {
        ...
        switch (nature) {
            case OPERATOR:
                canManage = getPermissionService().checkPermission("memberOperators", "manage");
                break;
            case MEMBER_RECORD:
                canManage = getPermissionService().checkPermission("systemMemberRecordTypes", "manage");
                break;
            case PAYMENT:
                final PaymentCustomField paymentField = (PaymentCustomField) field;
                ...
                if (paymentField.getTransferType().equals(backToTransferType)) {
                    ...
                    canManage = getPermissionService().checkPermission("systemAccounts", "manage");
                }
                request.setAttribute("backToTransferType", backToTransferType);
                break;
            default:
                canManage = getPermissionService().checkPermission("systemCustomFields", "manage");
                break;
        }
    ...
    protected void validateForm(final ActionContext context) {
        ...
        customFieldService.validate(field);
    }
