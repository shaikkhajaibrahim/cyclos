...
public class EditDynamicDocumentAction extends BaseFormAction {
    ...
    protected void checkPermission(final ActionContext context) {
        ...
        if (id > 0L) {
            document = (DynamicDocument) documentService.load(id);
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        if (document.isHasFormPage()) {
            final File formFile = CustomizationHelper.formFile(getServlet().getServletContext(), document);
            CustomizationHelper.updateFile(getServlet().getServletContext(), formFile, document.getFormPage());
        }

        final File documentFile = CustomizationHelper.documentFile(getServlet().getServletContext(), document);
        CustomizationHelper.updateFile(getServlet().getServletContext(), documentFile, document.getDocumentPage());
    ...
    private DynamicDocument retrieveDocument(final EditDynamicDocumentForm form) {
        ...
        if (formPage == null || StringUtils.isEmpty(formPage.getContents())) {
            ...
            final File formFile = CustomizationHelper.formFile(context, document);
        ...
        if (documentPage == null || StringUtils.isEmpty(documentPage.getContents())) {
            ...
            final File documentFile = CustomizationHelper.documentFile(context, document);
