...
public abstract class AbstractActionContext implements Serializable, MessageResolver {
    ...
    public AbstractActionContext(final ActionMapping actionMapping, final ActionForm actionForm, final HttpServletRequest request, final HttpServletResponse response, final User user) {
        if (actionMapping == null || request == null || response == null || user == null) {
    ...
    public void checkTransactionPassword(final String transactionPassword) {
        try {
            final User loggedUser = getUser();
            String member = null;
            if (loggedUser instanceof OperatorUser) {
                member = ((OperatorUser) loggedUser).getOperator().getMember().getUsername();
            }
            final AccessService accessService = SpringHelper.bean(getServletContext(), "accessService");
            accessService.checkTransactionPassword(member, loggedUser.getUsername(), transactionPassword, getRequest().getRemoteAddr());
    ...
    public boolean isTransactionPasswordEnabled() {
        ...
        if (loggedGroup instanceof OperatorGroup) {
            final FetchService fetchService = SpringHelper.bean(getServletContext(), "fetchService");
            loggedGroup = fetchService.fetch(loggedGroup, RelationshipHelper.nested(OperatorGroup.Relationships.MEMBER, Element.Relationships.GROUP));
        }
    ...
    public boolean isTransactionPasswordEnabled(final AccountType accountType) {
        if (!isTransactionPasswordEnabled()) {
            ...
            final GroupService groupService = SpringHelper.bean(getServletContext(), "groupService");
            try {
    ...
    public String message(final String key, final List<?> args) {
        return MessageHelper.message(getServletContext(), key, args);
    }
    ...
    protected void validateTransactionPassword(final TransactionPasswordStatus tpStatus) {
        switch (tpStatus) {
            case BLOCKED:
                throw new ValidationException("transactionPassword.error.blocked");
            case PENDING:
            case NEVER_CREATED:
