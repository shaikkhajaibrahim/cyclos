...
public class DisconnectUserAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        try {
            final User user = elementService.loadUser(userId);
            String param;
            String forward = "list";
            if (user instanceof AdminUser) {
                param = "adminId";
                getAccessService().disconnectLoggedAdmin((AdminUser) user, form.getSessionId());
                if (form.isBackToProfile()) {
                    forward = "adminProfile";
                }
            } else if (user instanceof MemberUser) {
                param = "memberId";
                getAccessService().disconnectLoggedMember((MemberUser) user, form.getSessionId());
                if (form.isBackToProfile()) {
                    forward = "memberProfile";
                }
            } else if (user instanceof OperatorUser) {
                param = "operatorId";
                getAccessService().disconnectLoggedOperator((OperatorUser) user, form.getSessionId());
                if (form.isBackToProfile()) {
