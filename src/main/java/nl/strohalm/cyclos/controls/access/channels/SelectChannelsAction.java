...
public class SelectChannelsAction extends BaseAction {
    ...
    protected ActionForward executeAction(final ActionContext context) throws Exception {
        ...
        if (context.getElement().equals(member)) {
            // Member selecting his/her own access to channels
            getAccessService().changeMyChannelsAccess(channels, true);
        } else {
            // Administrator or broker selecting the access to channels for a member
            getAccessService().changeMemberChannelsAccess(member, channels, true);
        }
