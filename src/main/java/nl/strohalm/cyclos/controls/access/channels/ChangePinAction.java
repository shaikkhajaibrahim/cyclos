...
public class ChangePinAction extends BaseFormAction {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        final MemberUser user = params.getUser();

        try {
            ActionForward forward;
            if (user == null || context.getUser().equals(user)) {
                getAccessService().changeMyPin(params, request.getRemoteAddr());
                forward = context.getSuccessForward();
            } else {
                getAccessService().changeMemberPin(params);
    ...
    protected void validateForm(final ActionContext context) {
        ...
        final MemberUser user = params.getUser();
        if (user == null || context.getUser().equals(user)) {
            getAccessService().validateChangeMyPin(params);
        } else {
            getAccessService().validateChangeMemberPin(params);
        }
    }
}