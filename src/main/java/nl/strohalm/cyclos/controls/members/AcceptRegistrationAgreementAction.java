...
public class AcceptRegistrationAgreementAction extends BasePublicFormAction {
    ...
    protected ActionForward handleSubmit(final ActionMapping mapping, final ActionForm actionForm, final HttpServletRequest request, final HttpServletResponse response) {
        ...
        if (registeredMember instanceof PendingMember) {
            ...
            try {
                elementService.setRegistrationAgreementAgreed(pendingMember, registrationAgreement);
            } catch (final DaoException e) {
                // The pending member has been removed
                final String validationKeyMessage = MessageHelper.message(request, "pendingMember.validationKey");
            ...
            final Member member = (Member) registeredMember;
            elementService.acceptAgreement(member, registrationAgreement, request.getRemoteAddr());
            return mapping.findForward("login");
        }
    }

    private RegistrationAgreement findAgreement(final RegisteredMember registeredMember) {
        final MemberGroup group = getFetchService().fetch(registeredMember.getMemberGroup(), MemberGroup.Relationships.REGISTRATION_AGREEMENT);
    ...
    private RegisteredMember findRegisteredMember(final ActionForm actionForm, final HttpServletRequest request) {
        ...
        final User loggedUser = (User) request.getSession().getAttribute("loggedUser");
        if (loggedUser == null) {
