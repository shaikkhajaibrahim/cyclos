...
public class EditGeneralReferenceAction extends BaseEditReferenceAction<GeneralReference> {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        if (context.getAccountOwner().equals(reference.getFrom())) {
            reference = referenceService.saveMyReference(generalReference);
        } else {
            reference = referenceService.saveMemberReference(generalReference);
        }
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (referenceId > 0L) {
            ...
            Member member;
            try {
                member = (Member) elementService.load(form.getMemberId());
        ...
        // Transaction feedbacks cannot be edited
        if (context.isAdmin()) {
            // An admin can manage both given and received
            editable = getPermissionService().checkPermission("adminMemberReferences", "manage");
        } else if (context.getElement().equals(reference.getFrom())) {
            // The member itself can manage given
            editable = getPermissionService().checkPermission("memberReferences", "give");
        } else if (context.isBrokerOf(reference.getFrom())) {
            // The member's broker can manage given
            editable = getPermissionService().checkPermission("brokerReferences", "manage");
        } else if (context.isOperator() && accountOwner.equals(reference.getFrom())) {
            // The member's operators can manage given
            editable = getPermissionService().checkPermission("operatorReferences", "manageMemberReferences");
        }
