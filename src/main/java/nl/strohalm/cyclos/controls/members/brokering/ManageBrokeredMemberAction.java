...
public class ManageBrokeredMemberAction extends BaseFormAction {
    ...
    protected ActionForward handleValidation(final ActionContext context) {
        try {
            ...
            final Member member = getFetchService().fetch(dto.getMember(), Member.Relationships.BROKER);
            final Map<String, Object> fields = new HashMap<String, Object>();
            final Member broker = member.getBroker();
            final Member newBroker = getFetchService().fetch(dto.getNewBroker());
            fields.put("currentBroker", broker == null ? "" : broker.getName());
            fields.put("newBroker", newBroker == null ? "" : newBroker.getName());
            fields.put("member", getFetchService().fetch(dto.getMember()).getName());
            ResponseHelper.writeStatus(context.getRequest(), context.getResponse(), Status.SUCCESS, fields);
