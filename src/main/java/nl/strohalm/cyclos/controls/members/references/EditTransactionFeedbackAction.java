...
public class EditTransactionFeedbackAction extends BaseEditReferenceAction<TransactionFeedback> {
    ...
    private Action resolveAction(final TransactionFeedback transactionFeedback, final ActionContext context) {
        if (transactionFeedback.isTransient()) {
            return Action.COMMENTS;
        } else {
            // Check the current feedback
            final TransactionFeedback current = (TransactionFeedback) referenceService.load(transactionFeedback.getId());
            if (context.isAdmin()) {
                return Action.ADMIN_EDIT;
            } else if (StringUtils.isEmpty(current.getReplyComments())) {
                return Action.REPLY_COMMENTS;
            } else {
                return Action.VIEW;
            }
        }
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        switch (resolveAction(feedback, context)) {
            case COMMENTS:
                feedback = referenceService.saveTransactionFeedbackComments(feedback);
                context.sendMessage("reference.transactionFeedback.saved");
                break;
            case REPLY_COMMENTS:
                feedback = referenceService.saveTransactionFeedbackReplyComments(feedback);
                context.sendMessage("reference.transactionFeedback.replyComments.saved");
                break;
            case ADMIN_EDIT:
                feedback = referenceService.saveTransactionFeedbackByAdmin(feedback);
                context.sendMessage("reference.transactionFeedback.saved");
                break;
        }
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (transferId > 0 || scheduledPaymentId > 0) {
            ...
            Payment payment;
            if (transferId > 0) {
                payment = paymentService.load(transferId, RelationshipHelper.nested(Transfer.Relationships.FROM, Account.Relationships.TYPE, AccountType.Relationships.CURRENCY), RelationshipHelper.nested(Transfer.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER));
            } else {
                payment = scheduledPaymentService.load(scheduledPaymentId, RelationshipHelper.nested(ScheduledPayment.Relationships.FROM, Account.Relationships.TYPE, AccountType.Relationships.CURRENCY), RelationshipHelper.nested(ScheduledPayment.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(ScheduledPayment.Relationships.TO, MemberAccount.Relationships.MEMBER));
            }
        ...
        if (feedbackId > 0) {
            try {
                transactionFeedback = (TransactionFeedback) referenceService.load(feedbackId, Reference.Relationships.FROM, Reference.Relationships.TO, RelationshipHelper.nested(TransactionFeedback.Relationships.TRANSFER, Transfer.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(TransactionFeedback.Relationships.TRANSFER, Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(TransactionFeedback.Relationships.TRANSFER, Transfer.Relationships.TYPE), RelationshipHelper.nested(TransactionFeedback.Relationships.SCHEDULED_PAYMENT, Transfer.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(TransactionFeedback.Relationships.SCHEDULED_PAYMENT, Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(TransactionFeedback.Relationships.SCHEDULED_PAYMENT, Transfer.Relationships.TYPE));
            ...
            final Calendar replyLimit = payment.getType().getFeedbackReplyExpirationTime().add(paymentDate);
            canReply = replyLimit.after(Calendar.getInstance()) && (context.getAccountOwner().equals(transactionFeedback.getTo()) && StringUtils.isEmpty(transactionFeedback.getReplyComments()));
            editable = getPermissionService().checkPermission("adminMemberTransactionFeedbacks", "manage");
        }
    ...
    protected void validateForm(final ActionContext context) {
        ...
        switch (resolveAction(transactionFeedback, context)) {
            case COMMENTS:
