...
public class MemberReferencesAction extends BaseQueryAction {
    ...
    protected QueryParameters prepareForm(final ActionContext context) {
        ...
        if (!isGeneral) {
            query.fetch(RelationshipHelper.nested(TransactionFeedback.Relationships.TRANSFER, Transfer.Relationships.TYPE, TransferType.Relationships.FROM, AccountType.Relationships.CURRENCY));
        }

        // When it's a member (or operator) viewing of other member's received general references, he can set his own too
        final boolean canSetReference = !context.isAdmin() && !context.getAccountOwner().equals(member) && isGeneral && getPermissionService().checkPermission("memberReferences", "give");

        // Check whether the logged user can manage references on the list
        boolean canManage = false;
        if (isGeneral) {
            // Only general references may be modified
            if (context.isAdmin()) {
                // An admin can manage both given and received
                canManage = getPermissionService().checkPermission("adminMemberReferences", "manage");
            } else if (isGiven && context.getElement().equals(member)) {
                // The member itself can manage given
                canManage = getPermissionService().checkPermission("memberReferences", "give");
            } else if (isGiven && context.isBrokerOf(member)) {
                // The member's broker can manage given
                canManage = getPermissionService().checkPermission("brokerReferences", "manage");
            } else if (isGiven && context.isOperator() && context.getAccountOwner().equals(member)) {
                canManage = getPermissionService().checkPermission("operatorReferences", isGeneral ? "manageMemberReferences" : "manageMemberTransactionFeedbacks");
            }
        }
