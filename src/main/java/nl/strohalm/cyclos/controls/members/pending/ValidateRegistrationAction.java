...
public class ValidateRegistrationAction extends BasePublicAction {
    ...
    protected ActionForward executeAction(final ActionMapping mapping, final ActionForm actionForm, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        // The only intresting session attribute for us is inContainerPage. All others will be cleared out
    ...
    private String findContainerUrl(final HttpServletRequest request, final PendingMember pendingMember) {
        ...
        final CustomizationData customization = CustomizationHelper.findCustomizationOf(getServlet().getServletContext(), Type.STATIC_FILE, group, null, "login.jsp");
        switch (customization.getLevel()) {
            ...
            case GROUP_FILTER:
                try {
                    final GroupFilter groupFilter = getFetchService().fetch(EntityHelper.reference(GroupFilter.class, customization.getId()));
