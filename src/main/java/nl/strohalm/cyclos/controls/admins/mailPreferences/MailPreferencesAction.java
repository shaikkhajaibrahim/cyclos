...
import nl.strohalm.cyclos.entities.members.preferences.MemberAlertNotificationPreference;
import nl.strohalm.cyclos.entities.members.preferences.SystemAlertNotificationPreference;
...
public class MailPreferencesAction extends BaseFormAction {
    ...
    /**
     * Transforms an AlertNotificationPreference in it's type
     * @author luis
     */
    private static class AlertTypeTransformer implements Transformer {

        private static final AlertTypeTransformer INSTANCE = new AlertTypeTransformer();

        public Object transform(final Object object) {
            if (object instanceof SystemAlertNotificationPreference) {
                return ((SystemAlertNotificationPreference) object).getType();
            } else if (object instanceof MemberAlertNotificationPreference) {
                return ((MemberAlertNotificationPreference) object).getType();
            }
            return null;
        }
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        group = getFetchService().fetch(group, AdminGroup.Relationships.VIEW_INFORMATION_OF, Group.Relationships.MESSAGE_CATEGORIES);

        // Find the initial groups this admin manages
        final List<MemberGroup> initialGroups = new ArrayList<MemberGroup>();
        for (final MemberGroup memberGroup : getFetchService().fetch(group, AdminGroup.Relationships.MANAGES_GROUPS).getManagesGroups()) {
        ...
        final List<TransferType> transferTypes = transferTypeService.search(transferTypeQuery);
        ...
        final TransferTypeQuery newPendingPaymentsQuery = new TransferTypeQuery();
        newPendingPaymentsQuery.setAuthorizable(true);
        final List<TransferType> newPendingPayments = transferTypeService.search(newPendingPaymentsQuery);

        List<GuaranteeType> guaranteeTypes = Collections.emptyList();
        if (getPermissionService().checkPermission("systemGuaranteeTypes", "view")) {
        ...
        try {
            ...
            request.setAttribute("selectedSystemAlerts", CollectionUtils.collect(notificationPreference.getSystemAlerts(), AlertTypeTransformer.INSTANCE));
            request.setAttribute("selectedMemberAlerts", CollectionUtils.collect(notificationPreference.getMemberAlerts(), AlertTypeTransformer.INSTANCE));
        ...
        request.setAttribute("initialGroups", initialGroups);
    ...
    private DataBinder<AdminNotificationPreference> getDataBinder() {
        if (dataBinder == null) {
            final BeanBinder<SystemAlertNotificationPreference> systemAlertBinder = BeanBinder.instance(SystemAlertNotificationPreference.class);
            systemAlertBinder.registerBinder("type", PropertyBinder.instance(SystemAlert.Alerts.class, "type"));
            final BeanBinder<MemberAlertNotificationPreference> memberAlertBinder = BeanBinder.instance(MemberAlertNotificationPreference.class);
