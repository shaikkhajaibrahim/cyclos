...
public class MembersReportHandler {
    ...
    private void adjustDto(final MembersTransactionsReportDTO dto) {
        ...
        final Collection<AccountType> accountTypes = fetchService.fetch(dto.getAccountTypes());
        dto.setAccountTypes(accountTypes);

        // Separate payment filters by account type
        final Collection<PaymentFilter> transactionsPaymentFilters = fetchService.fetch(dto.getTransactionsPaymentFilters(), PaymentFilter.Relationships.ACCOUNT_TYPE);
    ...
    private MembersTransactionsReportDTO readDTO(final ActionContext context) {
        ...
        dto.setTransactionsPaymentFilters(fetchService.fetch(dto.getTransactionsPaymentFilters(), PaymentFilter.Relationships.TRANSFER_TYPES));
        adjustDto(dto);
        return dto;
    }
