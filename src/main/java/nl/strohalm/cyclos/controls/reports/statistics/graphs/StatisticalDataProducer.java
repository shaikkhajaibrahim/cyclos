...
public class StatisticalDataProducer implements Serializable, DatasetProducer, CategoryToolTipGenerator, PieToolTipGenerator {
     ...
     * <ul>
     * <li> Changes keys to Strings by using the language resource bundle <li> fills up the list of values with "" so all columns have equal length.
     * </ul>
     * 
     ...
     * <ul>
     * <li>checks if a graph is needed <li>it may switch x and y, depending on the <code>resultObject</code>'s <code>dontSwitchXY</code> property <li>
     * generates the dataset <li>scales it.
     * </ul>
     */
