...
public class ExportMembersTransactionsReportToCsvAction extends BaseCsvAction {
    ...
    public MembersReportHandler getReportHandler() {
        if (reportHandler == null) {
            reportHandler = new MembersReportHandler(SettingsHelper.getLocalSettings(getServlet().getServletContext()));
    ...
    @Override
    protected CSVWriter<MemberTransactionSummaryReportData> initCsvWriter(final ActionContext context) {
        ...
        dto.setTransactionsPaymentFilters(getFetchService().fetch(dto.getTransactionsPaymentFilters()));
