...
public class ChargebackPaymentAction extends ViewTransactionAction {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        final Transfer transfer = paymentService.load(transferId, Transfer.Relationships.FROM);
        ...
        try {
            if (context.isMember()) {
                chargeback = paymentService.chargebackReceivedPayment(transfer);
            } else if (transfer.isFromSystem()) {
                chargeback = paymentService.chargebackSystemPayment(transfer);
            } else {
                chargeback = paymentService.chargebackMemberPayment(transfer);
            }
