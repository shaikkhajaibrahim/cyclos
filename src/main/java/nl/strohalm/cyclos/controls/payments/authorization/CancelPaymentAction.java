...
public class CancelPaymentAction extends BaseTransferAuthorizationAction {
    ...
    protected Transfer performAction(final ActionContext context) {
        ...
        if (context.getAccountOwner().equals(transfer.getFromOwner())) {
            if (context.isAdmin()) {
                transfer = transferAuthorizationService.cancelFromSystem(transferAuthorizationDto);
            } else {
                transfer = transferAuthorizationService.cancelFromMember(transferAuthorizationDto);
            }
        } else {
            transfer = transferAuthorizationService.cancelAsMember(transferAuthorizationDto);
        }
