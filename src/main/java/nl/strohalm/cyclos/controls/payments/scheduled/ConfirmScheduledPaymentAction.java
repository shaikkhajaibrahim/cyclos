...
public class ConfirmScheduledPaymentAction extends BaseFormAction {
    ...
    protected ActionForward handleSubmit(final ActionContext context) throws Exception {
        ...
        try {
            transfer = processTransfer(context, transfer);
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        final TransferType transferType = getFetchService().fetch(transfer.getType(), RelationshipHelper.nested(TransferType.Relationships.FROM, AccountType.Relationships.CURRENCY), TransferType.Relationships.TO);
    ...
    /**
     * Process the transfer itself calling the appropriate
     */
    private Transfer processTransfer(final ActionContext context, Transfer transfer) {
        if (transfer.getFromOwner().equals(context.getAccountOwner())) {
            // Member or admin processing a transfer from himself or operator processing a transfer from his member
            if (context.isAdmin()) {
                transfer = scheduledPaymentService.processTransferFromSystemToMember(transfer);
            } else if (transfer.getToOwner() instanceof SystemAccountOwner) {
                transfer = scheduledPaymentService.processTransferFromMemberToSystem(transfer);
            } else {
                transfer = scheduledPaymentService.processTransferFromMemberToMember(transfer);
            }
        } else {
            // An administrator or broker processing a transfer as a member
            if (transfer.getToOwner() instanceof SystemAccountOwner) {
                transfer = scheduledPaymentService.processTransferAsMemberToSystem(transfer);
            } else {
                transfer = scheduledPaymentService.processTransferAsMemberToMember(transfer);
            }
        }
        return transfer;
    }
    ...
    private Transfer resolveTransfer(final ActionContext context) {
        ...
        return paymentService.load(transferId, RelationshipHelper.nested(Transfer.Relationships.FROM, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(Transfer.Relationships.TO, MemberAccount.Relationships.MEMBER), RelationshipHelper.nested(Transfer.Relationships.SCHEDULED_PAYMENT, ScheduledPayment.Relationships.TRANSFERS));
    }

    private boolean shouldValidateTransactionPassword(final ActionContext context, final Transfer transfer) {
        final TransferType transferType = getFetchService().fetch(transfer.getType(), TransferType.Relationships.FROM);
