...
public class SimulateConversionAction extends BaseFormAction implements LocalSettingsChangeListener {
    ...
    private void attachLabels(final StatisticalResultDTO rawDataObject, final ConversionSimulationDTO dto, final ActionContext context) {
        ...
        transferType = getFetchService().fetch(transferType, TransferType.Relationships.TRANSACTION_FEES, RelationshipHelper.nested(TransferType.Relationships.FROM, AccountType.Relationships.CURRENCY));
    ...
    private DataBinder<ConversionSimulationDTO> getDataBinder() {
        try {
            ...
            if (dataBinder == null) {
                ...
                binder.registerBinder("date", PropertyBinder.instance(Calendar.class, "date", localSettings.getDateConverter()));
    ...
    private void reloadForm(final ActionContext context) {
        ...
        account = getFetchService().fetch(account);
        final List<TransferType> transferTypes = filterTransferTypesByAccount(account, allowedTTs);

        // get the default rates
        final ConversionSimulationDTO defaultDto = (myAccount) ? paymentService.getDefaultConversionDTOForSelf(account, transferTypes) : paymentService.getDefaultConversionDTOForMember(account, transferTypes);
    ...
    private Collection<TransferType> resolveAllowedTTs(final ActionContext context, final boolean myAccount) {
        ...
        if (!myAccount && loggedUserGroup instanceof BrokerGroup) {
            loggedUserGroup = getFetchService().fetch(loggedUserGroup, BrokerGroup.Relationships.BROKER_CONVERSION_SIMULATION_TTS);
            allowedTTs = ((BrokerGroup) loggedUserGroup).getBrokerConversionSimulationTTs();
        } else {
            loggedUserGroup = getFetchService().fetch(loggedUserGroup, Group.Relationships.CONVERSION_SIMULATION_TTS);
    ...
    private Member resolveMember(final ActionContext context) {
        ...
        if (memberId > 0) {
            try {
                member = (Member) elementService.load(memberId);
    ...
    private void showResults(final HttpServletRequest request, final ConversionSimulationDTO dto, final boolean myAccount) {
        ...
        final TransactionFeePreviewForRatesDTO result = myAccount ? paymentService.simulateConversionForSelf(dto) : paymentService.simulateConversionForMember(dto);
