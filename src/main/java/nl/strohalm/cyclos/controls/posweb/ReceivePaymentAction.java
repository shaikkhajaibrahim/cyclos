...
public class ReceivePaymentAction extends BasePosWebPaymentAction {
    ...
    protected Payment doPayment(final ActionContext context, final DoPaymentDTO dto) {
        ...
        Member from = (Member) dto.getFrom();
        try {
            from = getFetchService().fetch(from, Element.Relationships.USER);
            if (from == null) {
                throw new Exception();
            }
        ...
        if (!getAccessService().isChannelEnabledForMember(Channel.POSWEB, from)) {
        ...
        // "Login" as the member, to ensure the model layer will test permissions as that member
        LoggedUser.cleanup();
        LoggedUser.init(from.getUser());
        try {
            // Perform the payment itself
            return getPaymentService().doPaymentFromMemberToMember(dto);
        } finally {
            // Ensure the operator will "continue logged" for the current request
            LoggedUser.cleanup();
            LoggedUser.init(context.getUser());
        }
    }

    @Override
    protected String getOperationName() {
        return "externalReceivePayment";
    }
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        switch (credentials) {
            ...
            case CARD_SECURITY_CODE:
                ...
                break;
            case LOGIN_PASSWORD:
                numericCredentials = SettingsHelper.getAccessSettings(getServlet().getServletContext()).isNumericPassword();
                break;
            case TRANSACTION_PASSWORD:
                numericCredentials = StringUtils.containsOnly(SettingsHelper.getAccessSettings(getServlet().getServletContext()).getTransactionPasswordChars(), "0123456789");
        ...
        if (from instanceof Member) {
            ...
            if (StringUtils.isEmpty(form.getCredentials())) {
                ...
                if (!validation.hasErrors()) {
                    final Member member = getFetchService().fetch((Member) from);
    ...
    private ValidationException checkCredentials(final ActionContext context, final Member member) {
        ...
        if (StringUtils.isEmpty(credentials)) {
            ...
            try {
                ...
                getAccessService().checkCredentials(posWebChannel(), payer, credentials, context.getRequest().getRemoteAddr(), relatedMember);
    ...
    private String principalTypeLabel(final PrincipalType principalType) {
        ...
        switch (principal) {
            case USER:
                return MessageHelper.message(getServlet().getServletContext(), "posweb.client.username");
            case CUSTOM_FIELD:
                return principalType.getCustomField().getName();
            default:
                return MessageHelper.message(getServlet().getServletContext(), principal.getKey());
        }
    }
}
