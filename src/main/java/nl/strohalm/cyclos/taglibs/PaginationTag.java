...
public class PaginationTag extends TagSupport {
    ...
    public int doEndTag() throws JspException {
        ...
        try {
            ...
            if (!page.isEmpty()) {
                if (pageCount == 1) {
                    out.println(MessageHelper.message(pageContext.getServletContext(), "global.pagination.single.page", page.getTotalCount()));
                } else {
                    out.println(MessageHelper.message(pageContext.getServletContext(), "global.pagination.various.pages", page.getTotalCount()));
                    if (currentPage > 1) {
                        out.println(pageLink(currentPage - 1, "<img src=\"" + request.getContextPath() + "/pages/images/previous.gif\" style=\"valign:bottom\" border=\"0\" title=\"" + MessageHelper.message(pageContext.getServletContext(), "global.pagination.tooltip.previous") + "\">"));
                    }
                    out.println(StringUtils.join(resolvePages(currentPage, pageCount), ' '));
                    if (currentPage < pageCount) {
                        out.println(pageLink(currentPage + 1, "<img src=\"" + request.getContextPath() + "/pages/images/next.gif\" style=\"valign:bottom\" border=\"0\" title=\"" + MessageHelper.message(pageContext.getServletContext(), "global.pagination.tooltip.next") + "\">"));
                    }
                }
            }
            out.println("</span>");
