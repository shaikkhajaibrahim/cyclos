 ...
 * @author luis
 */
public class BrokeringServiceImpl implements BrokeringService {
    ...
    public List<Brokering> removeExpiredBrokerings(final Calendar time) {
        ...
        if (brokeringExpirationPeriod == null || brokeringExpirationPeriod.getNumber() <= 0) {
            return new ArrayList<Brokering>();
        }
        ...
        for (final Brokering brokering : expired) {
            // MessageAspect will send a message to each broker
            // Updates the brokering, expiring it
    ...
    private Brokering doChangeBroker(final ChangeBrokerDTO dto) {
        ...
        if (!justSuspendCommission) {
            ...
            if (member.equals(newBroker)) {
                throw new CircularBrokeringException();
            }
