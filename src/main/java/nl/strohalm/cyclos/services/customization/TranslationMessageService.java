...
public interface TranslationMessageService extends Service {
     ...
     */
    @AdminAction(@Permission(module = "systemTranslation", operation = "file"))
    Properties exportAsProperties();

    /**
     * Same as {@link #exportAsProperties()}, but with no permission check. Should be called from the resource bundle manager.
     */
    ...
    /**
     * Same as {@link #importFromProperties(Properties, MessageImportType)}, but with no permission check, and assuming
     * <code>importType == NEW_AND_EMPTY</code>. Should be called from system initialization, to ensure new translation versions will be imported
     */
    @SystemAction
     ...
     */
    @AdminAction(@Permission(module = "systemTranslation", operation = "manage"))
    int remove(Long... ids);
