...
public class DocumentServiceImpl implements DocumentService {
    ...
    public int removeDynamic(final Long... ids) {
        return documentDao.delete(ids);
    }

    public int removeMember(final Long... ids) {
        return documentDao.delete(ids);
    }

    public int removeStatic(final Long... ids) {
        return documentDao.delete(ids);
    }
    ...
    public DynamicDocument saveDynamic(DynamicDocument document) {
        ...
        if (isInsert) {
            ...
            AdminGroup group = (AdminGroup) groupService.load(LoggedUser.group().getId(), Group.Relationships.DOCUMENTS);
            ...
            if (!group.getPermissions().contains(documentDetails)) {
                group.getPermissions().add(documentDetails);
            }
            // Update the group
            group = groupService.updateAdmin(group);
            ...
            final Collection<Group> loadedGroups = loadedDocument.getGroups();
    ...
    public MemberDocument saveMember(final MemberDocument document, final InputStream stream, final int size, final String fileName, final String contentType) {
        return (MemberDocument) doSaveStatic(document, stream, size, fileName, contentType);
    }
    ...
    public List<Document> search(final DocumentQuery documentQuery) {
        if (LoggedUser.isValid()) {
            documentQuery.setViewer(LoggedUser.element());
        }
    ...
    private StaticDocument doSaveStatic(StaticDocument document, final InputStream stream, final int size, final String fileName, final String contentType) {
        ...
        if (document.isTransient()) {
            ...
            final AdminGroup group = (AdminGroup) groupService.load(LoggedUser.group().getId(), Group.Relationships.DOCUMENTS);
            ...
            groupService.updateAdmin(group);
        } else {
