...
/**
 * implementation of the <code>StatisticalTaxesService</code>.<br> This class is responsible of the gathering of the Taxes Statistics.
 * 
 * @author Rinke
 * 
 */

public class StatisticalTaxesServiceImpl extends StatisticalServiceImpl implements StatisticalTaxesService {
    ...
    protected Collection<AccountFee> getInitializedAccountFees(final StatisticalTaxesQuery query) {
        ...
        for (AccountFee accountFee : accountFees) {
            if (accountFee.getTransferType() == null) {
                accountFee = getFetchService().fetch(accountFee, AccountFee.Relationships.TRANSFER_TYPE);
                anyChanges = true;
            }
    ...
    protected Collection<TransactionFee> getInitializedTransactionFees(final StatisticalTaxesQuery query) {
        ...
        for (TransactionFee transactionFee : transactionFees) {
            if (transactionFee.getGeneratedTransferType() == null) {
                transactionFee = getFetchService().fetch(transactionFee, TransactionFee.Relationships.GENERATED_TRANSFER_TYPE);
                anyChanges = true;
            }
    ...
    public StatisticalResultDTO getSinglePeriodVolume(final StatisticalTaxesQuery query) {
        // TODO 1: PERIOD is not printed: voeg toe als param in titel of bij filters shown
        final Period periodMain = query.getPeriodMain();
        ...
        final TaxesPerMemberStats stats = new TaxesPerMemberStats(query, periodMain, getTransferDao(), getElementDao());
        StatisticalResultDTO result = null;
        final String baseKey = "reports.stats.taxes.singlePeriod.totalSum";
        ...
        int colCounter = 0;
        // TODO 5: check all stats op BigDecimal gevaren.
