...
public class MailHandler implements InitializingBean, DisposableBean {
    ...
    private static final ThreadLocal<MailSendingException> CURRENT_EXCEPTION = new ThreadLocal<MailSendingException>();

    /**
     * Clean the current mail sending exception, if any
     */
    ...
    /**
     * Returns the current mail sending exception
     */
    public static MailSendingException getCurrentException() {
        return CURRENT_EXCEPTION.get();
    }

    /**
     * Returns if there's a current mail sending exception
     */
    public static boolean hasException() {
        return CURRENT_EXCEPTION.get() != null;
    }
    ...
    /**
     * Sends an email now. The email is really sent after all the preceding ones in the queue have been sent.
     */
    public void sendEnqueue(final String subject, final InternetAddress replyTo, final InternetAddress to, final String body, final boolean isHTML) {
        if (senderThreads == null) {
            return;
        }
        final MailParameters params = new MailParameters();
    ...
    private void doSend(final String subject, final InternetAddress replyTo, final InternetAddress to, final String body, final boolean isHTML, final boolean throwException) {
        if (senderThreads == null || to == null || StringUtils.isEmpty(to.getAddress())) {
            return;
        }
        final LocalSettings localSettings = settingsService.getLocalSettings();
        final MailSettings mailSettings = settingsService.getMailSettings();
        ...
        try {
            ...
            CURRENT_EXCEPTION.set(new MailSendingException(subject, e));
