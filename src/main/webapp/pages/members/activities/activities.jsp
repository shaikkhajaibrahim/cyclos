            ...
            <table class="defaultTable">
            	<tr>
	            	...
	            	<c:if test="${member.group.broker}">
						...
						<td>${activities.numberBrokeredMembers}</td>
					</c:if>
                </tr>
               	
               	<c:if test="${showReferencesInformation}">
	               	<tr>
					...
					<c:forEach var="level" items="${referenceLevels}">
		            	<tr>
							...
							<td width="20%">${givenReferences[level]}</td>
							<td width="30%" class="headerLabel"><bean:message key="reference.level.${level}"/></td>
							<td width="20%">${receivedReferences[level]}</td>
		                </tr>
					</c:forEach>
				</c:if>
            	...
            	<tr>
					...
					<td>${activities.adsByStatus[adStatus['ACTIVE']]}</td>
					<td class="headerLabel"><bean:message key="ad.status.EXPIRED"/></td>
					<td>${activities.adsByStatus[adStatus['EXPIRED']]}</td>
                </tr>
            	<tr>
					<td class="headerLabel"><bean:message key="ad.status.PERMANENT"/></td>
					<td>${activities.adsByStatus[adStatus['PERMANENT']]}</td>
					<td class="headerLabel"><bean:message key="ad.status.SCHEDULED"/></td>
					<td>${activities.adsByStatus[adStatus['SCHEDULED']]}</td>
                </tr>
                
				<c:if test="${showInvoicesInformation}">
	               	<tr>
						<td colspan="4" class="label" style="text-align:left">
							<hr>
	            	...
	            	<tr>
						...
						<td>${activities.outgoingInvoices.count}</td>
	               	...
	               	<tr>
						...
						<td>${activities.incomingInvoices.count}</td>
...
<c:forEach var="entry" items="${activities.accountActivities}">
	...
	<c:if test="${showAccountInformation || accountActivities.hasRateInfo}">
	    	...
	    	<tr>
	        	<td align="left" class="tdContentTableForms" colspan="2">
	        		<cyclos:layout columns="4" className="defaultTable">
						<c:if test="${showAccountInformation && accountActivities.showAccountInfo}">
						...
						<c:if test="${accountActivities.hasRateInfo}">
							<c:if test="${not empty accountActivities.dRate}">
								<cyclos:cell width="30%" className="headerLabel"><bean:message key="account.dRate"/></cyclos:cell>
								<cyclos:cell><cyclos:format number="${accountActivities.dRate}"/>&nbsp;<bean:message key="global.timePeriod.DAYS"/></cyclos:cell>
							</c:if>
							<c:if test="${not empty accountActivities.aRate}">
								<cyclos:cell width="30%" className="headerLabel"><bean:message key="account.aRate"/></cyclos:cell>
								<cyclos:cell><cyclos:format number="${accountActivities.aRate}"/>&nbsp;<bean:message key="global.timePeriod.DAYS"/></cyclos:cell>
							</c:if>
							<cyclos:rowBreak/>
							<c:if test="${not empty accountActivities.totalFeePercentage}">
								<cyclos:cell width="30%" className="headerLabel"><bean:message key="activities.rates.asFeePercentage"/></cyclos:cell>
								<cyclos:cell><cyclos:format number="${accountActivities.totalFeePercentage}" precision="1" />&nbsp;%</cyclos:cell>
							    <cyclos:rowBreak/>
							</c:if>
						</c:if>
						...
						<c:if test="${showAccountInformation && accountActivities.showAccountInfo}">
		            		...
		            		<cyclos:cell>${accountActivities.remainingLoans.count}</cyclos:cell>
		                    ...
		                    <cyclos:cell>${accountActivities.debitsLast30Days.count}</cyclos:cell>
		                    ...
		                    <cyclos:cell>${accountActivities.creditsLast30Days.count}</cyclos:cell>
		                    ...
		                    <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.numberBuy"/></cyclos:cell>
		                    <cyclos:cell>${accountStatus.rootDebits.count}</cyclos:cell>
							<cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageBuy"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountStatus.rootDebits.average}"/></cyclos:cell>
	
		                    <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.numberSell"/></cyclos:cell>
		                    <cyclos:cell>${accountStatus.rootCredits.count}</cyclos:cell>
	                        <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageSell"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountStatus.rootCredits.average}"/></cyclos:cell>
		                    ...
		                    <c:if test="${member.group.broker}">
			                    ...
			                    <cyclos:cell>${accountActivities.brokerCommission.count}</cyclos:cell>
