...
<table class="defaultTable">
	...
	<c:forEach var="current" items="${references}">
		...
		<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
			...
			<c:if test="${showAmount}">
				<td valign="top" align="center" nowrap="nowrap" class="${isGiven ? 'ClassColorDebit' : 'ClassColorCredit'}"><cyclos:format number="${isGiven ? -current.transfer.amount : current.transfer.amount}" unitsPattern="${current.transfer.type.from.currency.pattern}" /></td>
			</c:if>
