...
Behaviour.register({
	...
	'img.fieldDetails': function(img) {
		...
		img.onclick = function() {
			self.location = pathPrefix + "/editCustomField?fieldId=" + img.getAttribute("fieldId") + "&memberRecordTypeId=" + getValue("memberRecordTypeId");
		}
	},
	...
	'img.remove': function(img) {
		...
		img.onclick = function() {
			if (confirm(removeConfirmationMessage)) {
				...
				self.location = pathPrefix + "/removeCustomField?fieldId=" + fieldId + "&memberRecordTypeId=" + memberRecordTypeId;
			}
		}
	}
