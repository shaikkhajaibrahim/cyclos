function enableBuyersAndSellersPermissions() {
	mdd_canBuyWithPaymentObligationsFromGroups.enable();
	mdd_canBuyWithPaymentObligationsFromGroups.render();
	$('chk_sellWithPaymentObligations').disabled = false;
}

function disableBuyersAndSellersPermissions() {
    mdd_canBuyWithPaymentObligationsFromGroups.values.each(function(option) { option.selected = false; } );
	mdd_canBuyWithPaymentObligationsFromGroups.disable();
	mdd_canBuyWithPaymentObligationsFromGroups.render();
	$('chk_sellWithPaymentObligations').checked = false;
	$('chk_sellWithPaymentObligations').disabled = true;
}

function enableIssuersPermissions() {
	mdd_guaranteeTypes.enable();
	mdd_guaranteeTypes.render();
	mdd_canIssueCertificationToGroups.enable();
	mdd_canIssueCertificationToGroups.render();
}

function disableIssuersPermissions() {
	mdd_guaranteeTypes.values.each(function(option) { option.selected = false; } );
	mdd_guaranteeTypes.disable();
	mdd_guaranteeTypes.render();
	mdd_canIssueCertificationToGroups.values.each(function(option) { option.selected = false; } );
	mdd_canIssueCertificationToGroups.disable();
	mdd_canIssueCertificationToGroups.render();
}
...
function showHideAll() {
	if (typeof(mdd_guaranteeTypes) == 'undefined') {
		return;
	} 
	mdd_guaranteeTypes.render();
	mdd_canIssueCertificationToGroups.render();
...
function hasIssuersPermissions() {
	return (hasOptionSelected(document.getElementsByName('permission(canIssueCertificationToGroups)')) || hasOptionSelected(document.getElementsByName('permission(guaranteeTypes)')));
}

function hasBuyersAndSellersPermissions() {
	return (hasOptionSelected(document.getElementsByName('permission(canBuyWithPaymentObligationsFromGroups)')) || $('chk_sellWithPaymentObligations').checked);
}
...
Behaviour.register({
	...
	'#chk_sellWithPaymentObligations': function(checkbox){
		checkbox.onclick = function(checkbox) {
			showHideIssuersPermissions();
		}
	},
