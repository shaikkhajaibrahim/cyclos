    ...
    <tr>
		<td align="left" class="tdContentTableForms" colspan="2">
		  	<table class="defaultTable" border="0">
		  		<tr>
           			<td class="label">
           				<span class="label"><bean:message key="group.nature"/></span>
		  			...
		  			<c:if test="${not empty groupFilters}" >
	           			<td class="label">
	           				<span class="label"><bean:message key="member.groupFilter"/></span>
            ...
            <table class="defaultTable">
            	...
            	<c:forEach var="group" items="${groups}">
            		<c:if test="${(cyclos:name(group.nature)=='ADMIN' && cyclos:granted('systemAdminGroups', 'view')) ||
                                  ...
                                  (cyclos:name(group.nature)=='OPERATOR' && cyclos:granted('memberOperators', 'manage'))}">
		            	<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
		            		...
		            		<td align="center" nowrap="nowrap">
		            			<c:choose><c:when test="${cyclos:name(group.nature)=='OPERATOR' || cyclos:granted('systemGroups', permissionByNature[group.nature])}">
		            			...
		            			<c:choose><c:when test="${group.status.enabled && (cyclos:name(group.nature)=='OPERATOR' || cyclos:granted('systemGroups', permissionByNature[group.nature]))}">
		                      	...
		                      	<c:if test="${cyclos:name(group.nature)=='OPERATOR' || cyclos:granted('systemGroups', permissionByNature[group.nature])}"> 
