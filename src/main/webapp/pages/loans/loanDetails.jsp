...
<c:if test="${not empty repaymentAmounts and (canRepay or canDiscard or canPerformExpiredAction)}">	
	<c:set var="loanPayment" value="${repaymentAmounts.loanPayment}"/>
	<table class="defaultTableContent" cellspacing="0" cellpadding="0">
		<tr>
			<td class="tdHeaderTable"><bean:message key="loan.title.${canRepay or canDiscard ? 'repayment' : 'canPerformExpiredAction'}" /></td>
			...
			<td colspan="2" align="left" class="tdContentTableForms">
				...
				<table class="defaultTable">
					<c:if test="${canRepay or canDiscard}">				
						...
						<c:if test="${cyclos:granted('adminMemberLoans', 'repayWithDate')}">
							<tr>
