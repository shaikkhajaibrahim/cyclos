...
<table width="100%">
	...
	<c:if test="${not empty query.expiration.begin}">
		<tr>
			...
			<td class="printData"><cyclos:format date="${query.expiration.begin}"/></td>
		</tr>
		<tr>
			<td class="printLabel"><bean:message key="card.expirationPeriod.to"/>:&nbsp;</td>
			<td class="printData"><cyclos:format date="${query.expiration.end}"/></td>
		</tr>
	</c:if>
...
<c:forEach var="card" items="${cards}">
	<cyclos:layout columns="4" className="printContent" width="100%">
		...
		<cyclos:cell className="printData" width="40%"><cyclos:format date="${card.expirationDate}"/></cyclos:cell>
