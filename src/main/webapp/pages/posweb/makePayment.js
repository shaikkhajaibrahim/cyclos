...
function onMemberUpdate(member) {
	...
	if (isEmpty(member) || isEmpty(currency)) {
		...
		params['channel'] = 'web';
		params['fromOwnerId'] = loggedMember;
		params['currencyId'] = currency;
		params['toOwnerId'] = member.id;
...
function updateTransactionPasswordCallback(request) {
	var isUsed = request.responseXML.getElementsByTagName("isUsed").item(0).firstChild.data;

	$('passwordRow')[isUsed == "true" ? 'show' : 'hide']();		
}
