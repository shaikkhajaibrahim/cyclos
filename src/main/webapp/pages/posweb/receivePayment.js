...
function onMemberUpdate(member) {
	...
	if (memberNameDisplay) memberNameDisplay.value = member.name;
	...
	if (!isEmpty(member) && !isEmpty(currency)) {
		...
		params['channel'] = 'posweb';
		params['fromOwnerId'] = member.id;
		params['currencyId'] = currency;
		params['toOwnerId'] = loggedMember;
...
function createSchedulingFields() {
	...
	if (document.maxScheduledPayments > 0) {
		...
		if (!allowsScheduling) {
			maxScheduledPayments = 0;
		}
	}
	
	var schedulingTypesToUse = [];
	if (maxScheduledPayments > 0) {
...
function loadMember(principal) {
	...
	new Ajax.Request(context + "/do/loadMemberAjax", {
	    method: 'post',
		parameters: params,
