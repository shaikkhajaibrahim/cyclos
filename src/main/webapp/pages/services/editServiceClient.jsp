        ...
        <td colspan="2" align="left" class="tdContentTableForms">
            <table class="defaultTable">
				...
				<tr id="tr_credentials_required" class="operationTR" style="display:none">
	 				...
	 				<td width="20px"><input type="checkbox" disabled="disabled" class="checkbox" id="chk_credentialsRequired" name="serviceClient(credentialsRequired)" value="true" <c:if test="${serviceClient.credentialsRequired}">checked="checked"</c:if> /></td>
				</tr>
                <tr>
                   			...
                   			<c:choose>
										...
										<td>
											<table class="nested" width="100%">
												<tr>
													<td width="30%" nowrap="nowrap"><bean:message key="serviceOperation.${operation}"/></td>
													<td>
														<cyclos:multiDropDown varName="manageGroups" name="serviceClient(manageGroups)" size="5" disabled="true">
                   				...
                   				<c:otherwise>
									<%-- The SMS operation only appears if SMS notifications are enabled. Others always appear --%>
                   					<c:if test="${cyclos:name(operation) != 'SMS' || localSettings.smsNotificationEnabled}">
	                   					<tr id="tr_${operation}" class="operationTR" style="display:none">
			                   				<td width="20px"><input type="checkbox" disabled="disabled" class="checkbox" id="chk_${operation}" name="serviceClient(permissions)" value="${operation}" <c:if test="${cyclos:contains(serviceClient.permissions, operation)}">checked="checked"</c:if> /></td>
