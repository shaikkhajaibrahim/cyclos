...
<c:set var="canManagePasswords" value="${cyclos:granted('adminAdminAccess', 'changePassword')}"/>
<c:set var="managePasswordsKey" value="profile.action.manageLoginPassword"/>
<c:if test="${admin.group.basicSettings.transactionPassword.used && cyclos:granted('adminAdminAccess', 'transactionPassword')}">
        ...
        <td colspan="2" align="left" class="tdContentTableForms">
			<cyclos:layout className="defaultTable" columns="4">
                ...
                <c:forEach var="entry" items="${countByRecordType}">
               	...
               	<c:if test="${cyclos:granted('adminAdmins', 'changeGroup')}">
		   	    ...
		   	    <c:if test="${disabledLogin && cyclos:granted('adminAdminAccess', 'enableLogin')}">
	    	    ...
	    	    <c:if test="${isLoggedIn && cyclos:granted('adminAdminAccess', 'disconnect')}">
