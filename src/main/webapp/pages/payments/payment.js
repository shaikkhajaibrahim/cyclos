...
function fetchTransferTypes() {
	...
	params["channel"] = "web";
	params["context"] = "PAYMENT";
	params["showCurrency"] = "true";
	params["fromOwnerId"] = isEmpty(from) ? "0" : from;
	if (isToSystem) {
		params["toNature"] = "SYSTEM";
	} else {
		params["toOwnerId"] = memberId;
	}
	params["useBy"] = "true";
	params["scheduling"] = "true";
...
function calculateMultiplePayments(callback) {
	var params = $H();
	params['from'] = getValue('from');
	params['amount'] = getValue('amount');
	params['date'] = getValue('dateText');
	params['firstPaymentDate'] = getValue('firstPaymentDateText');
	params['paymentCount'] = getValue('paymentCountText');
	params['recurrence.number'] = getValue('recurrenceNumberText')
	params['recurrence.field'] = getValue('recurrenceFieldSelect') 
	...
	'#backButton': function(button) {
		button.onclick = function() {
			...
			if (booleanValue(params.get("fromQuickAccess"))) {
				...
				self.location = pathPrefix + "/profile?memberId=" + ifEmpty(getValue("to"), getValue("from"));
			}
		}
	},
