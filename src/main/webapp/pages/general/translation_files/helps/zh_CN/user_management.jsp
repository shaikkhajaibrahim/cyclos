...
<p>如果您正在新增的会员的组别已存在有权限的经纪组别，您可以（可选择）从经纪组别指定一个经纪给该会员。为此，您应该拣选底部的标示为 &quot;指定经纪&quot; 的复选框。<br>
您可以指定用户的密码，并指定用户可以直接使用密码或在下次登录时改变它。
<p>在填写数据后，您可以选择储存会员和输入新的会员（按钮 &quot;储存并新增会员&quot;），或直接开启新会员的 <a href="${pagePrefix}profiles"><u>个人资料</u></a>（按钮 &quot;储存和开启个人资料&quot;）。
<hr class="help">
</span>
...
<p>如果您想寻找一个会员，您可以填写各种搜索字段（但没有一个是强制性的）。如果您只点击 &quot;搜索&quot;
...
<p>您可以选择打印搜索结果：点击在右上角视窗帮助图标旁边的打印图标（<img border="0" src="${images}/print.gif" width="16" height="16">）。
这会将您带到一个友好的打印机视窗，在这里您可以按一下按钮，打印概况。
<p>另一种可能性是下载结果文件：点击储存图标（<img border="0" src="${images}/save.gif" width="16" height="16">）。
结果将下载为 <a href="${pagePrefix}loans#csv"><u>CSV 格式</u></a>，其中将包含所有存在于会员 <a
...
<p>注：在报告功能，您能够检索更具体的 <a href="${pagePrefix}reports#member_lists"><u>会员名单</u></a>，例如会员名单包含更多的数据，如账户余额和广告数目。
<hr class="help">
</span>
...
在等待某一特定时间后，待审批会员将被自动从系统（和名单）删除。所有三个注册办法（在公众注册页面自我注册、由经纪注册和会员注册）都可以要求电邮确认。<br>
...
<p>形式非常简单：您可以指定一个关键字，和/或搜索某个管理员组别。如果您只点击 &quot;搜索&quot; 按钮，没有输入任何字段，您会得到一份所有管理员的名单。<br>
<p>为了注册一个新的管理员，您必须使用表格底部的下拉框。在这里，您选择新的管理员的 <a
...
<p>表格不言自明。任何有红星（*）的字段是强制性的。
<p><p>在输入数据后，您可以选择储存管理员和输入一个新的管理员（按钮 &quot;储存并新增管理员&quot;）或直接开启新管理员的 <a href="${pagePrefix}profiles"><u>个人资料</u></a>（按钮 &quot;储存并开启个人资料&quot;）。
<hr class="help">
</span>
...
<ul>
	...
	你可以通过标记一个或两个选择框，为已拥有卡的会员“再创建”卡类型。
	你可以选择为已有卡在等待和/或活跃状态的会员再创建卡。
	在这两种情况下，现有的卡将被取消。
</ul>
<hr class="help">
...
<h3>导入会员</h3>
这个功能可以导入会员（个人资料），并可选择设置每名会员的初始信贷，并生成从或给系统账户的初始付款。数目和日期的格式应根据当地的设置定义。<br>
字段由栏的名称所确定。名称不区分大小写，必须在第一行。字段可在任何栏（不存在第一或第二栏，只要是名称正确便可）。如果一个字段是可选的，您可以省略整个栏或用空栏值。<br>
可支持以下栏/字段。
<ul>
	<li>名称（必填）<br>
	会员姓名。
	<li>用户名（必填）<br>
	这是登录名，必须是唯一的，否则会给予错误。如果登录名是自动生成的（账户编码），没有必要定该栏，联环系统将生成登录名。
	<li>新增日期（可选）<br>
	如果拣选它，个人成员报告的日期（注册日期）将会用此日期。如果没有日期，将会用现在的（导入）日期。
	<li>密码（可选）<br>
	会员密码。会员在第一次登入时必须改变它的。
	<li>电邮（根据联环系统设置决定是必填或可选的）。它必须是一个良好的电邮地址。
	<li>余额（可选）<br>
	最初的账户余额。只在已拣选账户类型时才用。如果您选择账户类型，您可以指定付款类型（负余额是会员付给系统，正余额是系统付给会员）。<br>
	如果您使用此选项，您必须确保将被扣除的账户有足够的信贷。
	<li>信贷额度（可选）<br>
	账户的（负）信贷额度。如果是空，将采取组别账户设置的默认值。
	<li>信贷额度上限（可选）<br>
	账户的信贷额度上限。如果是空，将采取组别账户设置的默认值。
	<li>自定字段的内部名称（可选）<br>
	有关选定组别的自定字段的内部名称。将会执行必填的验证。如果字段是一个名单（列举值），只有在会员已有现存的数值时，才能成功导入。
	例如，如果您有一个自定字段 &quot;地区&quot;，有三种可能的地区 &quot;中区&quot;、&quot;南区&quot; 和 &quot;北区&quot;，其他地区将不得导入。空白地区的会员将被导入（除非设置为
	&quot;必填&quot;）。
	<li>会员记录类型自定字段的内部名称（可选）<br>
	会员记录的数值。默认数据库的一个例子是 &quot;Remark.comments&quot;；意见是备注记录类型的一个字段，这个字段必须是（在字段配置指定的）内部名称。<br>
...
如果您选择 &quot;导入&quot;，将导入成功的会员。，但先查看导入会员的状态，仍是好的。
<hr class="help">
...
<ul>
	...
	视窗如下。
	<li>首先执行 <a href="${pagePrefix}#search_member_by_member"><u>搜索会员</u></a>（&quot;菜单：搜索&quot;）。
