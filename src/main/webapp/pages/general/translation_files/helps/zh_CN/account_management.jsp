...
该默认的数据库可以延伸至多个账户和转让类型。我们建立了一个会员账户和各种不同的系统账户。
...
<ul>
	...
	<br>
	<li><b>社区帐户：</b> 社区账户是社区拥有的账户，它可以接收税款（如果已配置）和会员支付缴款。
...
<ul>
	...
	<ul>
		<li><b>反馈意见期限：</b> 当完成付款后，付款人（＝买家）可以设置反馈意见的最长时间。这期限可以在此设定。
		<li><b>答复反馈意见期限：</b> 反馈意见的接收人（＝卖家）可以答复反馈意见。这最长期限可以在此设定。
...
<ul>
	...
	&quot;授权&quot;。<br>
	这项授权类型是相当罕见的。如果系统已使用 <a href="${pagePrefix}invoices"><b>发票</b></a>，我们建议不要使用此选项，因为发票提供类似的功能。
...
<ul>
	...
	<br>
	<li><b>支付者</b>：在这里您可以定义谁将被收取费用，这可能是系统账户的来源（付款人）、目标（收款人）或指定会员。<br>
	<br>
	<li><b>接收者</b>：在这里您可以定义谁将收到费用，这可能是系统账户的来源（付款人）、目标（收款人）或指定会员。<br>
	<br>
	<li><b>允许任何账户</b>：正常情况下，在下面一项（&quot;生成交易类型&quot;）中，只显示同一种货币的有关交易类型。如果您拣选此复选框，则没有限制可套用于交易费用中的货币：所有交易类型会显示，即使是另一种货币。<br>
	<br>
	...
	<br>
	<li><b>金额</b>：在这里，您可以填写费用的金额。如果您想费用的金额是原本交易金额的一个百分比，您必须在选择框中选择百分比符号﹪。
	如选择 &quot;固定&quot;，将收取一个与原本交易金额无关的固定金额。<br>
	<br>
...
<ul>
	...
	<br>
	<li><b>第一个收费期段开始：</b> 在这里选择需要开始的第一个收费期段；您可能想要选择日期拣选工具，请单击日历 <img border="0" src="${images}/calendar.gif" width="16"
		height="16">&nbsp;图标。<br>
	完成后，您可能想要在 &quot;菜单：账户 > 账户费用&quot; 拣选下一次执行。<br>
	<br>
	<li><b>收费模式</b>：有五种可能的收费模式：
	<ul>
		<li><b>固定金额</b>：固定金额可以是缴款（会员付款给系统账户）或系统账户一次性或预定付款给会员组别。
		<li><b>正使用量百分比</b>：正使用量百分比的计算方法与利息的计算方式相同。实际上，当定义付款方向（见下文）为 &quot;系统付给会员账户&quot; 时，这将是利息。
		如果方向是 &quot;会员付给系统账户&quot; 费用将是转换利息（滞期费）。<br>
		到期金额是根据期段和金额。会员的余额在整个月份有100个单位，收取1 ％的利息/滞期费将是1单位。
		<li><b>负使用量百分比</b>：负使用量百分比以同样的计算方式运作，但只发生在负结余。
		<li><b>正账户余额百分比</b>：账户余额百分比费用不计算数量（时间+余额），只计算余额。当执行费用时（手动或预定），费用将只考虑收取时候的余额。
		这就意味着，如果在上面的例子中，在费用执行之前，若会员降低他的余额为零，他将不会支付任何费用。账户余额百分比可能会导致系统发生不正常的行为，但这是一个方法问题。
	...
	<br>
	<li><b>基本免费额</b>：如果有基本免费额，可以在超过一定数额后才实行收费。<br>
	...
	注：“启用日期”只能设置一次。该选项一旦被使用，将不会显示。当然你可以随时手动禁用/启用上面的“启用”选项。
	<br>
