...
例如，如果组织需要一个额外的 <a
	href="${pagePrefix}profiles"><u>个人资料</u></a> 字段来指出会员鞋子的大小，管理员可以增加一个新的字段，并定义新增字段的各种属性，如：名称、类型、大小、可见性、权限、位置、验证和其他行为和设置。<br>
字段可以分配到 <a href="${pagePrefix}groups"><u>组别</u></a>。让您有不同的注册表格和个人资料给消费者和企业，这只是一个例子。在上述的例子中，它让组织为会员创建不同的政策和不同的鞋子大小。<br>
联环系统配备一套默认自定字段，它当然也是易于管理的。当然，并非所有在数据库中的字段都是自定字段；一些字段是相当重要，使得它们不能被删除，也不能被修改。<br>
</p>
<i>在哪里可以找到它？</i><br>
...
会员、管理员和操作员字段将会显示在他们的个人资料。联环系统附带的默认数据库已经有一些定义了的自定会员字段。<br>
贷款和贷款小组字段将显示在贷款和贷款小组页面，而广告字段将显示在广告页面。
<p>字段的名称和配置可以通过选择修改图标进行修改。在帮助文件的页面，您可以找到关于自定字段配置的详细资料。<br>
您可以点击红色的删除图标来删除字段。请注意，删除自定字段的唯一可能是它未被使用；当数据库中的任何字段有资料，将无法删除该字段。在这种情况下，您可以通过清除所有组别的拣选来隐藏该字段。
<p>选择 &quot;更改字段次序&quot; 提交按钮，提供您选项以更改出现在页面的字段次序。<br>
...
<ul>
	...
	<li><b>可见者（只是广告）：</b> 在这里你可以定义谁可以查看广告自定字段。这可以是如下：<br>
	所有用户（所有者以及所有其他会员、经纪和管理员；所有经纪和管理员；或只是管理员。
	<li><b>在搜索显示（只有付款字段）：</b> 如果选择此项，自定支付字段将在账户摘要显示为筛选器。如付款方式是贷款，它将显示在管理员的搜索贷款。
	<li><b>列表显示结果（只有付款字段）：</b> 如果选择此项，自定支付字段将在搜索结果列表显示为一栏。如付款方式是贷款，它将显示在管理员的贷款搜索结果列表。<br>
	...
	<li><b>可以隐藏会员：</b> 在这里，您可以定义谁可以选择隐藏字段。（权限运作的等级结构与查看权限相同）
	<li><b>在会员打印显示：</b> 在这里，您可以定义字段会显示在谁人的搜索会员打印页面。（权限运作的等级结构与查看权限相同）
	<li><b>验证：</b> 您可以指定以下验证：
	<ul>
...
<h3>自定字段值</h3>
这个视窗列表显示字段的可能值。
<p>您可以选择删除图标来删除数据值，这只适用于特定的数据值不在使用中。&quot;空&quot; 值是有可能的，通过分配所有已使用的值到另一列表的值。（这在修改数据值会进一步解释）
<p>
...
<ul>
	...
	<li><b>默认：</b> 如果选择此项，当显示表单时，数据值将预先选定。每个自定义字段只能有一个默认数据值。
