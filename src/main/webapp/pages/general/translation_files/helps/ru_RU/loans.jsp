<a name="top"></a>
<p class="head_description">Ссуды служат одним из кредитных механизмом в Системе. Ссуды выдаются администрацией по запросу участников. Система поддерживает основные настройки ссуд, включая планирование платежей в оплату тела и процентов по ссуде, позволяет также применять различные сборы. Следует осторожно обходиться с выдачей ссуд, так как это легко может дестабилизировать Систему. Например: при массовой выдаче ссуд при малом предложении товаров и услуг.</p>
...
<p><i>Где это найти / как это работает?</i><br>
...
<ol>
	...
	<p>Новый тип платежа: "выплата взносов по ссуде" - <b>loan repayment</b> создается следующим образом:
	<ul>
...
<p>Если вы выберите ячейку &quot;Является ссудой&quot;, то <a
...
<p>Есть три типа <a href="#loan_types"><u>ссуд</u></a>
...
<a name="loan_types"></a>
<h3>Типы ссуд</h3>
<p>В системе есть три <a href="#top"><u>типа ссуд</u></a>:
...
<p>Нужно заполнить следующие поля, чтобы выдать ссуду: 
<ul>
...
<p>Примечание: Можно создавать <a href="${pagePrefix}custom_fields"><u>дополнительные поля формы ссуд</u></a> при необходимости. Например, номер ссудного договора. 
<hr class="help">
</span>
...
<p>Значок печати (<img border="0" src="/pages/images/print.gif" width="16"
...
<p>Если ссуда относится к типу <a href="#loan_types"><u>Ссуда в рассрочку</u></a> (включая сложные ссуды), откроются еще несколько полей. Эти поля недоступны для простых ссуд. 
...
<p><span class="admin">Используйте либо кнопку "Выплатить", либо "Списать ссуду/взнос". </span> <span class="member">нажмите на кнопку &quot;Выплатить&quot;, чтобы произвести выплату (взноса) ссуды.</span>
<hr class="help">
...
<a name="component"></a>
<b>Взнос</b>
<p>Один из числа последовательных платежей по погашению задолженности. Когда погашение ссуды разделено на несколько частей, каждая из этих частей называется взносом. 
<hr class='help'>
...
<b>CSV</b>
файлы
<p>CSV означает &quot;разделенные запятой значения&quot;; это формат для файлов данных, которые можно скачать из различных результатов поиска в окнах системы. В этом формате значения полей разделены запятыми
