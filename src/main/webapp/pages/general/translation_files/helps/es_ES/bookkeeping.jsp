...
administrativo de las transacciones de Cyclos con las importadas. Esto significa
que ambas transacciones tienen referencias entre si y por ende tienen un estado extra. 
Este estado muestra un ícono específico en el resumen de transacciones. Es también
posible buscar e imprimir listados con resultados de transacciones por estado.

<p>Por lo general este módulo se utiliza para controlar saldos de sistemas donde las
unidades internas (de Cyclos) son respaldadas con dinero convencional en cuentas bancarias.
En estos casos las transacciones bancarias tienen relación directa con transacciones 
específicas en Cyclos. Por ejemplo, un deposito (pago entrante) en el Banco puede estar
relacionado con un pago de sistema-a-miembro que puede ser una &quot;compra&quot; de Unidades
de Cyclos o un repago de un préstamo. Y en el caso contrario, un pago de una cuenta bancaria
(débito) a una cuenta bancaria del miembro puede estar relacionada a un pago miembro-a-sistema
en Cyclos (en caso de una conversión). La función de conciliación ayuda al control del
...
<p>Es posible generar automáticamente tipo de transacciones (Cyclos)
...
<p>El control del &quot;respaldo&quot; es solamente un ejemplo del uso de este módulo.
Puede utilizarse también para otros casos donde se necesitan transacciones externas 
para generar transacciones locales o cambio de estado de préstamos en el sistema local.</p>
...
Para que el módulo sea visible debe tener configurado los <a
	href="${pagePrefix}groups#manage_group_permissions_admin_system"><u>permisos
de administración</u></a> correctos para las &quot;Cuentas Externas&quot;.
<p>Antes de poder importar transacciones externas a cyclos, debe realizar los siguientes
pasos:
<ol>
	<li>Verifique que los <a
		href="${pagePrefix}account_management#transaction_types"><u>
	tipos de transacciones</u></a> que necesita para estas transacciones existan.
	<li>Cree una cuenta externa en &quot;Menú: Administrar Cuentas Externas >
	Nueva cuenta externa&quot;.
	<li><b>mapear archivo:</b> Crear una definición del archivo de importación en la ventana
	abajo de en la cual creo la cuenta externa.
	<li><b>mapeo de archivo:</b> Especifique en cyclos como tratar los campos de este archivo
	en la próxima ventana.
	<li><b>mapeo de tipo de pago:</b> Especifique en cyclos como se mapean los valores del
	campo que define el tipo de pago con los tipos de pago de cyclos. Esto se hace en la 
	siguiente ventana.
</ol>
Solo después de hacer estos pasos (con uno es suficiente) podrá realizar la importación de
transacciones externas mediante el archivo de transacción recibido de si cuenta bancaria.
<a href="#using"><u>Haga click aquí</u></a> para ver como importar transacciones desde
un archivo.<hr>
...
En esta página usted puede modificar o crear una cuenta externa interna, 
seleccionando la cuenta de sistema y la cuenta de miembro que estarán relacionadas a la misma.<br>
<br>
...
Luego de crear la cuenta externa se le pedirá crear el archivo de mapeo, donde podrá ingresar las siguientes opciones:
<ul>
...
Si hace click en el botón &quot;Inicializar&quot;, se eliminan todos los archivos de mapeo y se le
solicitará la creación de uno nuevo. Los tipos de pago (en la ventana ubicada debajo)
no serán eliminados al inicializar el mapeo de archivo.
<hr class="help">

<A NAME="file_mapping_fields_list"></A>
<h3>Archivo de campos de mapeo</h3>
Una vez definido el formato del mapeo de archivo (ventana superior), se pueden mapear los
campos del archivo de transacciones a campos de Cyclos. 
...
Debe repetir esto para mapear todos los campos en el archivo de transacciones.
<br>
...
<p>
Una vez ingresado un archivo a la lista, varias acciones son posibles:
<ul>
...
Puede ingresar un Nombre, que es solo una etiqueta que se mostrará en la lista (no posee ninguna función).<br> 
...
<ul>
	...
	depósitos, pagos de préstamo, etc.<br>
	...
	ubicada debajo de la ventana de "Archivo de campos de mapeo".
...
<h3>Configurar orden de campos de mapeo</h3>
...
Paga agregar un tipo de pago, debe existir el campo de mapeao correspondiente al Campo: "Tipo de pago". <br>
...
Aquí se especifica que tipo de transacción específica se mapea con que código. 
...
<a href="#edit_field_mapping"><u>mapeos del campo tipo de pago</u></a>. Vea la lista ubicada debajo con un ejemplo.<br>
<br>
