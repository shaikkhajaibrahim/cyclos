...
<p>Eine weitere Möglichkeit ist Herunterladen des Ergebnisses in eine Datei, durch Anklicken 
des Speichern-Symbols <img border="0" src="${images}/save.gif" width="16" height="16">&nbsp;.
Das Ergebnis wird im <a href="${pagePrefix}loans#csv"><u>CSV-Format</u></a>, heruntergeladen, 
...
wollen.
<p>Anmerkung: In der Berichte-Funktion können Sie spezifischere 
...
Mit dieser Funktion ist es möglich Mitglieder (Profile) zu importieren und optional je Mitglied 
ein Kreditlimit anzugeben und eine Zahlung (Saldo) von oder zu einem Systemkonto zu generieren. 
Zahlen und Datum müssen entsprechend den Basiseinstellungen formatiert werden.<br>
Die Felder werden über den Spaltennamen identifiziert. Die Spaltennamen müssen in der ersten Zeile (Kopfzeile)
stehen, Groß-Kleinschreibung wird nicht beachtet. Die Spaltennamen können in beliebiger Reihenfolge 
angeordnet werden (es gibt keine Spalte A oder B), solange die Spaltennamen korrekt angegeben werden wird 
importiert.
Bei einem optionalen Feld kann die gesamte Spalte entweder weggelassen oder der Inhalt der Spalte leer 
gelassen werden. <br>
Die CSV Einstellungen (z.B. Trennzeichen) müssen entsprechend den Basiseinstellungen für CSV-Dateien 
gewählt werden, der Zeichensatz ist Unicode (UFT-8).<br>
Die folgenden Felder (Spalten) werden unterstützt.

<ul>
	<li>name (erforderlich)<br>
	Der Mitgliedsname.
	<li>username (erforderlich)<br>
	Dies ist der Benutzername, er muss eindeutig sein, es kommt zu einem Fehler, wenn der Benutzername
	...
	Das Erstellungsdatum des Mitglieds. Dieses erscheint im persönlichen Mitgliedsbericht (Mitglied seit). 
	Wenn kein Datum angegeben wird, wird das aktuelle Datum des Imports eingetragen. 
	<li>password (optional)<br>
	Das Kennwort. Die Mitglieder werden aufgefordert das Kennwort bei der ersten Anmeldung zu ändern.
	...
	Der Anfangssaldo des Kontos. Wird nur verwendet, wenn ein Kontotyp ausgewählt ist. Falls Sie einen
	...
	Das Kreditlimit der Konten (negativ). Bei leerem Eintrag wird das Standardlimit des Kontos der 
	Gruppeneinstellungen verwendet.
	<li>uppercreditlimit (optional)<br>
	Das obere Kreditlimit der Konten. Bei leerem Eintrag wird das Standardlimit des Kontos der 
	Gruppeneinstellungen verwendet.
	<li>benutzerdefiniertes Feld interner Name (optional)<br>
	Der interne Name des benutzerdefinierten Feldes bezogen auf die ausgewählte Gruppe. Die erforderliche
	Gültigkeit wird geprüft. Ist das Feld eine Liste (aufzählend) ist der Import nur erfolgreich
	...
	<li>Mitgliedseintrag.benutzerdefiniertes Feld interner Name (optional)<br>
	Werte für Mitgliedseinträge. Als Beispiel für das Standard-Projekt (Deutsch) ist &quot;Bemerkung.Kommentare&quot;. 
	Kommentare ist ein Feld vom Eintrag Bemerkung. Diese Angabe ist der interne Name des 
	Feldes(spezifiziert	in der Konfiguration des benutzerdefinierten Feldes).<br>Zu beachten ist, wenn Sie Mitgliedseinträge 
	importieren wollen, dass alle Felder die in diesem Mitgliedseintrag	definiert sind eine eigene Spalte 
	in der CSV-Datei benötigen.
	</ul>
<hr class="help">
...
<p><A NAME="contacts"></A>
<h3>Kontakte</h3>
<p class="head_description">Über diese Seite können Sie Ihre Kontakte verwalten.</p>
...
<ul>
	...
	im unteren Fenster.
	<li>Über <a href="${pagePrefix}#search_member_by_member"><u>Suche eines Mitglieds</u>
