...
<span class="admin">
<p><i>Wie werden die Berichte aktiviert?</i><br>
...
Falls es sich bei den Daten um Ihre eigenen handelt, so können Sie auch die Daten in Zusammenhang mit Ihren 
<a href="${pagePrefix}invoices"><u>Rechnungen</u></a> zeigen.
<hr class="help">

<span class="admin">
<a name="current_state"></a>
<h3>Zustandsbericht</h3>
Diese Funktion ermöglicht es Ihnen, eine Übersicht über den 
derzeitigen Status des Systems zu erhalten.
<a href="#current_result"><u>Klicken Sie hier</u></a> um eine Übersicht der möglichen Daten zu erhalten..
<p>Verwenden Sie die Kontrollkästchen um auszuwählen, was Sie zeigen möchten, und klicken Sie dann auf
...
<p>Falls Sie eine größere Anzahl Kontrollkästchen anklicken, dauert es möglicherweise 
einige Sekunden, bis die Berechnungen fertig sind. Haben Sie also bitte etwas Geduld.<p>
...
<p>ANMERKUNG: Bitte füllen Sie das Formular in der vorgegebenen Reihenfolge aus, 
von oben nach unten. Der Zahlungsfilter kann z.B. leer sein, wenn Sie nicht 
zuvor einen Kontotyp gewählt haben. <p>
Am unteren Ende der Seite können Sie den Befehl zum <a href="#results"><u>Bericht drucken</u></a>
oder <a href="#results"><u>Bericht herunterladen</u></a> der Ergebnisse geben.
<p>
...
<ul>
	...
	<a href="${pagePrefix}loans#csv"><u>CSV-Format</u></a> herunter.
</ul>
<hr class="help">
</span>
...
<a href="${pagePrefix}settings#local"><u>Basiseinstellungen</u></a> nach.</span><p>
Dieses Fenster ermöglicht Ihnen einen Überblick darüber, welche SMS-Nachrichten 
...
<p class="sub_description">
<h2>Simulationen</h2>
Der Simulationsbereich von Berichten produziert nicht wirklich Berichte im Sinne
von Daten aus der Datenbank des Systems. Der
Simulationsbereich ermöglicht Ihnen die Untersuchung von Simulationen bestimmter
Konfigurationen in Cyclos, speziell der Systeme von d-rates und 
a-rates

</p>
Weiter Information erhalten Sie unter
<a href="${pagePrefix}account_management#rates"><u>rates</u></a>
, oder
<a href="${pagePrefix}account_management#a-rate"><u>a-rate</u></a>
und
<a href="${pagePrefix}account_management#d-rate"><u>d-rate</u></a>
<hr>

<A NAME="choose_simulation"></A>
<h3>Simulation wählen</h3>
Diese Seite ermöglicht die Auswahl von Simulationen. Die folgenden Simulationen
sind verfügbar:
<ul>
	<li><b>Konfiguration der D-rate</b>: Ermöglicht Ihnen, die Konfiguration der <a
		href="${pagePrefix}account_management#d-rate"><u>D-rate</u></a>
	anzusehen. Dies zeigt das Verhältnis zwischen der Konversionsgebühr
	und der d-rate (oder Datum).
</ul>
<hr class="help">

<span class="admin"> <a name="dRate_config_simulation"></a>
<h3>D-rate Konfigurations-Simulation</h3>
Diese Seite ermöglicht Ihnen die Kurve anzusehen, die aus einer bestimmten Konfiguration
der <a href="${pagePrefix}account_management#d-rate"><u>D-rate</u></a>
Parameter resultiert. Diese Parameter werden für die <a
	href="${pagePrefix}account_management#currency_details"> <u>Währung</u></a> eingestellt.
Die Kurve zeigt das Verhältnis zwischen der d-rate (Datum) un des sich aus 
dieser d-rate prozentuell ergebenden Gebührenanteils.
<ul>
	<li><b>Parameter</b> Eingabefelder: Diese Felder bestimmen die Parameter 
	für die Konfiguration der d-rate. Ist bereits eine Konfiguration 
	verfügbar, sind die Bearbeitungsfelder mit diesen Werten befüllt. Diese können
	allerding jederzeit geändert werden. <br>
	Hat Ihr System mehr als eine Währng mit freigegebener d-rate,
	müssen Sie im Auswahlfeld zuerst die Währung bestimmen.
	Ist nur eine Währung mit freigegebener d-rate verfügbar, wird dieses Auswahlfeld
	nicht erscheinen.<br>
	Für die Bearbeitung der restlichen Felder lesen Sie bitte die Erklärung in <a
		href="${pagePrefix}account_management#currency_details"><u>
	Währung ändern</u></a>.<br>
	<br>
	<li><b>Spanne</b> Eingabefelder:<br>
	Dies ermöglicht Ihnen die Eingrenzung der zu zeigenden Grafik.
	<ul>
		<li><b>Beginnen mit D =:</b> Der am weitesten links gelegene Punkt auf der X-Achse
		der Grafik. Der höchste Wert für D wird ganz links dargestellt, da D
		mit der Zeit abnimmt und die X-Achse die Zeit repräsentiert.
		<li><b>Beenden mit D =:</b> Der am weitesten rechts gelegene Punkt der X-Achse
		der Grafik. der niedrigste Wert für D wird ganz rechts dargestellt, da D
		mit der Zeit abnimmt.
	</ul>
</ul>
Nach anklicken von &quot;Weiter&quot; wird die Grafik 
im <a href="#reportsSimulationsDRateConfigGraph"><u>Ergebnisfenster</u>
</a> angezeigt.
<hr class="help">

<A NAME="reportsSimulationsDRateConfigGraph"></A>
<h3>D-Rate Konfigurationskurve</h3>
Diese Grafik zeigt die Kurve einer D-rate Konfiguration.<br>
Die X-Achse zeigt die sich verringernde <a
	href="${pagePrefix}account_management#d-rate"><u>D-rate</u></a>. Da die
D-rate mit der Zeit abnimmt, zeigt die Grafik auch das zunehmende Datum.<br>
Die Y-Achse zeigt den prozentuelle Gebührenanteil der sich aus dieser D-rate ergäbe.
<p>Gegen Sie mit dem Mauszeiger auf einen Punkt der Grafik um die damit 
verbundenen Daten anzusehen.
<hr class="help">

<A NAME="aRate_config_simulation"></A>
<h3>A-Rate Konfigurations-Simulation</h3>
Diese Seite ermöglicht es Ihnen, die Kurve anzusehen, die aus einer bestimmten Konfiguration
der <a href="${pagePrefix}account_management#a-rate"><u>A-rate</u></a>
Parameter resultiert. Diese Parameter werden für die <a
	href="${pagePrefix}account_management#transaction_fee_details"> <u>Überweisungsgebühr</u></a>
eingestellt. Die Kurve zeigt das Verhältnis zwischen a-rate (Datum) und es sich aus 
dieser a-rate ergebenden prozentuellen Gebühr. Es ist möglich, sowohl die
Kurven der Konfiguration für &quot;A-rated&quot; Gebühren, als auf für
&quot;Kombinierte Gebühren A/D&quot; zu zeigen.
<ul>
	<li><b>Überweisungstyp zur Verwendung mit Standardwerten:</b> Dieses Formular versucht
	bereits existierende, mit a-rated verbundene Überweisungsgebühren in Ihrem System zu finden, um
	deren Parameter in die Formularfelder einzugeben, so dass
	Sie die Konfigurationskurven für tatsächlich existierende Überweisungsgebühren
	zeigen können. Gibt es mehr als eine Überweisungsgebühr mit freigegebener
	A-rate für mehr als einen Überweisungstyp, so werden Sie im Formular
	gebeten den zu verwendenden Überweisungstyp zu bestimmen. Gibt es nur einen einzigen
	Überweisungstyp mit a-rated Gebühr(en), so wird diese Feld nicht erscheinen.
	<li><b>Überweisungsgebühr zur Verwendung mit Standardwerten:</b> Analog zum
	vorherigen Punkt: Gibt es mehr als eine Überweisungsgebühr mit A-rate
	für diesen Überweisungstyp, so werden Sie im Formular gebeten dies zu spezifizieren.
	Andernfalls erscheint das Bearbeitungsfeld nicht.<br>
	<li><b>Parameter</b> Eingabefelder: Diese Felder bestimmen die Parameter für
	die Konfiguration der A-rate. Ist bereits eine Konfiguration verfügbar, 
	sind die Bearbeitungsfelder mit diesen Werten befüllt. Diese können allerding
	jederzeit geändert werden.<br>
	Für die Bearbeitung der Felder unter &quot;Parameter&quot; lesen Sie bitte die Erklärung in
  <a href="${pagePrefix}account_management#transaction_fee_details"><u>
	Währung ändern</u></a>. Bitte beachten Sie, dass Sie natürlich nur solche Buchungstypen
	wählen können, die eine A-rate verwenden.<br>
	<br>
	<li><b>Spanne</b> Eingabefelder:<br>
	Dies ermöglicht Ihnen die Eingrenzung der zu zeigenden Grafik.
	<ul>
		<li><b>Beginnen mit A =:</b> Der am weitesten links gelegene Punkt auf der X-Achse
		der Grafik. Im Falle eines Buchungstypes mit &quot;gemischten A/D rates&quot;,
		ist dies als Prozentsatz für die gesamte Sicherheitsperiode angegeben. Zum
		Beispiel: Beläuft sich der gesamte Sicherheitsperiode auf 80 Tage, so entspricht
		ein A von 20 dem 25%-Punkt auf der X-Achse (da 20 ja 25% von 80 sind).<br>
		Ist der Buchungstyp &quot;A-rate&quot;, so wird dieses Feld einfach 
		in Tagen ausgedrückt, der normalen Einheit für die A-rate.
		<li><b>Beenden mit A =:</b> Der am weitesten rechts gelegene Punkt auf der X-Achse
		der Grafik. Analog zum verherigen Punkt. 
	</ul>
</ul>
Nach anklicken von &quot;Weiter&quot; wird die Grapik im
<a href="#reportsSimulationsARateConfigGraph"><u>Ergebnisfenster</u></a> angezeigt.
<hr class="help">

<A NAME="reportsSimulationsARateConfigGraph"></A><br> 
<A NAME="reportsSimulationsARateConfigCombinedGraph"></A>
<h3>A-rate Konfigurationskurve</h3>
Diese Grafik zeigt die Kurve einer A-rate Konfiguraton einer 
Überweisungsgebühr.<br>
Die X-Achse zeigt die <a href="${pagePrefix}account_management#a-rate"><u>A-rate</u></a>.
Da die A-rate mit der Zeit zunimmt, zeigt die Grafik auch das zunehmende Datum.<br>
Im Falle einer Gebühr mit &quot;gemischten A/D-rated&quot;, ist die X-Achse als Prozentsatz
für die gesamte Sicherheitsperiode angegeben. Zum Beispiel: Beläuft sich die
gesamte Sicherheitsperiode auf 80 Tage, so entspricht eine A von 20 dem 
25%-Punkt auf der X-Achse (da 20 25% von 80 sind).

In case of a &quot;Mixed-A/D-rated&quot; fee, the x-axis is expressed as
a percentage of the total guarantee period. For example: if the total
guarantee period is 80 days, an A of 20 will represent the point on the
x-axis of 25% (as 20 is 25% of 80).<br>
Die Y-Achse zeigt den prozentuellen Gebührenanteil der sich aus dieser A-rate ergäbe.
<p>Gegen Sie mit dem Mauszeiger auf einen Punkt der Grafik um die damit 
verbundenen Daten anzusehen.
