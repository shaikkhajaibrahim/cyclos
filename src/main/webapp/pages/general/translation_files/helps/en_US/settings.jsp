...
<b>SMS: </b>
<p>If you enable SMS Cyclos will allow payments, invoices and
consulting account information via SMS. The SMS module in Cyclos
requires an external SMS controller. This software is still being
developed and at the time of writing no publishing is planned yet. You
are always welcome to use your own SMS controller.<br>
The communication between Cyclos and SMS controller will happen via
webservices. Therefore you will need to provide a webservice URL. You
will also need to add a channel for SMS payments (in &quot;Menu:
Settings - Channels&quot;). Once that SMS option is active it will show
up in the group settings and as a channel in notification settings.
...
<ul>
	...
	<ul>
		...
		<li><b>Identifier lenght: </b> The length of identifier
		(seguential)
		<li><b>Suffix: </b> Suffix of the identifier (numbers or letters)

		
	</ul>
	For example: the transtion number of the first transaction in the
...
<p>Take much care if you charge back a transaction that has related
loans. This might result in errors if the loan has been repaid.

<p><b>Scheduled tasks: </b> This setting is normally used for
...
<ul>
	...
	login attempts an alert is generated. In case the user does exists a
	member alert will be generated. In case the member does not exists a
	system alert will be generated
</ul>
<hr>
...
the changes click &quot;submit&quot;. <br>
The following options are available:
<ul>
	<li><b>Use virtual keyboard: </b> When this option is checked the
	users (both admin and member) will be presented a virtual keyboard at
	the login page. The password needs to be typed using the virtual
	keyboard. A virtual keyboard prevents malicious key logging software
	...
	<li><b>Allow operator login:</b> If you have <a
		href="${pagePrefix}operators"><u> operators</u></a> enabled, this
	checkbox must be checked in order to allow operators to login.
	<li><b>Admin. session timeout: </b> The time an administrator will
	be logged out after being inactive.
	...
	addresses or hostnames of the users that can access the administration
	section. Please put every hostname or ip on a new line (return) If you
	leave the &quot;#Any host&quot; entry empty, any host will be allowed
	to access the administration login page (but user name and password is
	still needed)
	<li><b>Login name generation: </b>
	<ul>
		<li><b>Manual by member: </b> For community networks it is common
		for users to choose their own login name or &quot;nickname&quot;. In
		this case the option &quot;Manual by Member&quot; needs to be
		selected.<br>
		If this option is selected you can specify the minimal and maximal
		login length and a regular expression to force the format of the login
		name.<br>
		...
		<li><b>Generated random number (login number): </b> For business
		networks it is common to use generated login names or &quot;account
		numbers&quot;. When this module is activated the register forms do not
		have an login name input field. When the form is submitted a login
		name (random number) will automatically be created. Below this option
		you can specify the length of the generated login code.
	</ul>
	...
	transaction password generation. (for the transaction password settings
	refer to the <a href="${pagePrefix}groups#edit_member_group"><u>group
	settings</u></a>).
</ul>
<hr>
...
In order to make changes click the &quot;change&quot; button; to save
the changes click &quot;submit&quot;.
<p>The following configurations are possible:
<ul>
...
It is unlikely that you will need to configure channels. For the webPOS,
POS and custom channels you can define the way user are identified.<br>
For any other channel you can define the confirmation credentials. (This
option is not available for the main web access because in that case it
is defined in the group self)
<hr>
...
accessed.
<p>You can edit a webservice selecting the edit icon and delete on
