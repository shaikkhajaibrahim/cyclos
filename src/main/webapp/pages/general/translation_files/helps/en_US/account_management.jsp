...
<p class="head_description">All configurations relating to accounts
and transactions can be done in the accounts management functions.</p>
...
can always be extended with more accounts and transfer types. 
...
<p>The database comes with the following default System Accounts:
<ul>
...
<p>To create a new member or system account you will have to click
...
<p>Note1: The permissions of a transaction type are defined per
group and can be set in &quot;Menu: Users & Groups > Permission groups >
edit group permissions.&quot;
<p>Note2: If the selected account is a member account, the specific
...
<ul>
	...
	member&quot; option explained above. <br>
...
help</u></a>.
<p><b><a href="#account_fees"><u>Periodic Fees</u></a></b>
<ul>
...
<p><b><a href="#transaction_fees"><u>Transaction Fees</u></a></b>.
...
authorization</u></a>.
<p>There are various possible levels of authorization, and there can
...
<p>This window will show all the authorization levels for the
...
<p>If you un select the authorization option in the transaction type
...
<p>If an authorization payment generates other payments (e.g. a loan
...
<ul>
	...
	option if <a href="${pagePrefix}invoices"><b>invoices</b></a> are used
...
<ul>
	...
	charged. This can be a system account the source (payer), destination
	(receiver) or a fixed member.<br>
	<br>
	<li><b>Will receive</b>: Here you can define who will receive the
	fee. This can be a system account the source (payer), destination
	(receiver) or a fixed member.<br>
	<br>
	...
	the previous edit). If you chose anything else in the previous edit,
	this field will not be visible.<br>
	<br>
...
<p>The administration and overview of the account fees is done from
...
<p>The window will show a list with the active (enabled) account
fees , the last and next execution date and their status
(&quot;failed&quot;, &quot;succesful&quot;).

<p>Account fees can be either automatic (scheduled) or manual.
Manual account fees run only when the link &quot;Run now&quot; is
clicked. While the account fee run is in progress it is possible to
cancel the account fee by clicking the &quot;cancel&quot; link. <br>
A canceled fee or a fee that failed because of technical problems will
always perform a &quot;roll back&quot;. This means that you will never
have half finished account fees. After the account fee has run
successfully the last execution date will be set and an entry will show
up in the <a href="#account_fee_history"><u>account fee history
list</u></a> (window below).

<p>If an account fee has failed an alert will show up in the alert
log and the status will show that the fee was failed. A failed fee can
be resumed by clicking the &quot;recharge&quot; link in the account fee
history list.
...
fees</u></a> as done in history. If an account fee charging has failed
the status will show that the fee was failed. A failed fee can be
resumed by clicking the &quot;recharge&quot; link.
<hr class="help">
...
<p>The following configuration options are available</b>
...
<ul>
	...
	this case: charging will not be possible if this box is not checked.<br>
	<br>
	<li><b>First charged period starts at:</b> choose here when the
	first charge period needs to start. You may want to choose the date
	picker by clicking the calendar <img border="0"
		src="${images}/calendar.gif" width="16" height="16">&nbsp; icon.
	<br>
	When done, you may want to check the next execution at &quot;Menu:
	Accounts > account fees&quot;. <br>
	<br>
	...
	<ul>
		...
		system account&quot; the fee will be converted interest (demurrage).<br>
		The amount due is based on a period and amount. An interest/demurrage
		of 1% charged over a member balance that has 100 Units during the
		entire month will be 1 unit.
	...
	fee only above a certain amount.<br>
...
<p>With this function, individual credit limits can be set for the
...
<p>The upper credit is a rarely used function. When the member
