...
<p class="head_description">Alle configuratie met betrekking to rekeningen en transacties 
geschiedit in het rekeningenbeheer</p>
...
<p>Cyclos ships with a database with a set of <a href="#standard_accounts"><u>
...
<p>To create a new member or system account you will have to click the
...
transaction type.
<p>Note1: The autorisaties of a transaction type are defined per group and
can be set in &quot;Menu: Users & Groups > Permission groups > edit group
autorisaties.&quot;
<p>Note2: If the selected account is a member account, the specific account
...
help</u></a>.
<p><b><a href="#account_fees"><u>Periodic Fees</u></a> (= Contribution
and Liquidity tax):</b>
<ul>
...
<p><A NAME="geautoriseerd_payment_levels"></A>
...
<p>There are various possible levels of authorization, and there can be more
...
<p>This window will show all the authorization levels for the transaction
...
<p>If you un select the authorization option in the transaction type it
...
<p>If an authorization payment generates other payments (e.g. a loan with
...
<p>The administration and overview of the account fees is done from the
...
<p>The window will show a list with the active (enabled) account fees , the
last and next execution date and their status (&quot;failed&quot;,
&quot;succesful&quot;).
<p>Account fees can be either automatic (scheduled) or manual. Manual
...
<p>If an account fee has failed an alert will show up in the alert log and
...
to make payments.
<p>The upper credit is a rarely used function. When the member reaches the
